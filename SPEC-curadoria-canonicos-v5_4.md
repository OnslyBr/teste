...
1. Patch no `domain_synonyms.json` v2.0 → v3.0 com 58 entradas novas. Baseline real de 113 entradas existentes. Total final: 171.
...
10. Migração SQL com novas colunas em `job_postings`: `description_hash`, `curation_source`, `prompt_structure_version`, `prompt_content_version`, `human_validated`, `canonical_resolved_at`, `canonical_role_label`, `layer_2_hint_count`.
...
Esta v5.3 **não** resolve os seguintes problemas, deliberadamente:
...
2. **Promoção automática de canônicos `pending` para `active`.** 533 canônicos em pending hoje. v5.3 **aceita pending no lookup** (Camada 1 emite flag) mas **exige aprovação explícita do Onsly** (via §2.4 + whitelist `allowedForPreResolution`) para auto-decisão sem LLM. Automação de promoção baseada em vacancy_count ou quórum temporal fica para sprint separada.
3. **Revisão humana obrigatória para domínios novos.** Erros Tipo 4 do diagnóstico (~12%) — vagas cujo domínio não tem canônico correspondente — continuam dependendo de decisão manual do Onsly. Pipeline v5.3 não cria canônicos automaticamente para domínios inéditos; marca como `pending` e aguarda curadoria.
4. **Convergência semântica em títulos ambíguos fora do patch §6.1.** As 58 entradas novas cobrem os casos de alto volume identificados empiricamente. Títulos ambíguos não cobertos (ex: "Solution Consultant" — delivery vs pre-sales) continuam dependendo do LLM guiado pelas Regras A-E.
5. **Auditoria retroativa do catálogo legado.** Backfill da v5.3 grava `prompt_structure_version='legacy'` em vagas anteriores (exceto `human_validated=true`), marcando-as como não-elegíveis para servir de âncora na Camada 0. Correção retroativa de erros sistêmicos pré-v5.3 é trabalho manual do Onsly (procedimento análogo ao Canonical GSI) ou sprint futura de batch recuration.
6. **Consolidação das CHECK constraints duplicadas em `curation_status`.** Conhecido mas não endereçado — v5.3 usa apenas a interseção efetiva dos dois CHECKs existentes.
...
Colunas esperadas em `job_postings`:

- `original_language text`
- `original_title text`
- `original_description text`
- `curation_in_progress boolean`
- `curation_lock_acquired_at timestamptz`

Colunas esperadas em `analyses`:

- `error_code text`

Colunas esperadas em `ai_usage_logs`:
...
WHERE table_schema = 'public'
  ...
  AND column_name IN (
      'original_language', 'original_title', 'original_description',
      'curation_in_progress', 'curation_lock_acquired_at'
  );
-- Esperado: 5 linhas retornadas
```

Sem essas colunas, o retrofit de `ai_usage_logs.metadata` e `original_*` já feito em código TypeScript quebra em runtime.
...
VALUES
    (gen_random_uuid(), 'cto', 'CTO', 'active', 'manual'),
    (gen_random_uuid(), 'ceo', 'CEO', 'active', 'manual'),
    (gen_random_uuid(), 'coo', 'COO', 'active', 'manual'),
    (gen_random_uuid(), 'cmo', 'CMO', 'active', 'manual'),
    (gen_random_uuid(), 'cfo', 'CFO', 'active', 'manual')
...
│ • SEM fallback por embedding nesta v5.3                      │
└─────────────────────────────────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────────────┐
...
CREATE TABLE job_postings_backup_v5_4 AS
    SELECT *, NOW() AS backup_taken_at
    FROM job_postings;

CREATE TABLE job_canonical_roles_backup_v5_4 AS
    SELECT *, NOW() AS backup_taken_at
    FROM job_canonical_roles;

CREATE TABLE skill_merge_decisions_backup_v5_4 AS
    SELECT *, NOW() AS backup_taken_at
    FROM skill_merge_decisions;

CREATE TABLE role_merge_decisions_backup_v5_4 AS
    SELECT *, NOW() AS backup_taken_at
    FROM role_merge_decisions;

-- Validação pós-backup: contagens devem bater 1:1 com originais
DO $$
DECLARE
...
BEGIN
    ...
    SELECT COUNT(*) INTO bkp_jp FROM job_postings_backup_v5_4;
    SELECT COUNT(*) INTO orig_jcr FROM job_canonical_roles;
    SELECT COUNT(*) INTO bkp_jcr FROM job_canonical_roles_backup_v5_4;
    SELECT COUNT(*) INTO orig_smd FROM skill_merge_decisions;
    SELECT COUNT(*) INTO bkp_smd FROM skill_merge_decisions_backup_v5_4;
    SELECT COUNT(*) INTO orig_rmd FROM role_merge_decisions;
    SELECT COUNT(*) INTO bkp_rmd FROM role_merge_decisions_backup_v5_4;
    ...
    RAISE NOTICE 'Backup validado: job_postings=%, job_canonical_roles=%, skill_merge_decisions=%, role_merge_decisions=%',
        bkp_jp, bkp_jcr, bkp_smd, bkp_rmd;
END $$;
```
...
-- Se precisar, rodar:
-- TRUNCATE job_postings; INSERT INTO job_postings SELECT ... FROM job_postings_backup_v5_4;
...
```sql
DROP TABLE IF EXISTS job_postings_backup_v5_4;
DROP TABLE IF EXISTS job_canonical_roles_backup_v5_4;
DROP TABLE IF EXISTS skill_merge_decisions_backup_v5_4;
DROP TABLE IF EXISTS role_merge_decisions_backup_v5_4;
```
...
**Mudança em `upsertCanonicalRole` (§7.11):** ao criar canônico novo, popular `prompt_version` com o valor do `PROMPT_STRUCTURE_VERSION` em uso naquele momento. Na prática, uma linha adicional no INSERT:

```typescript
// lib/pipeline/persist-curation.ts — upsertCanonicalRole
// Em todos os INSERTs em job_canonical_roles, adicionar:
prompt_version: PROMPT_STRUCTURE_VERSION,  // v5.4 — rastreabilidade de drift
```
...
### 6.1 `domain_synonyms.json` — bump 2.0 → 3.0 com 58 entradas novas

Entradas a adicionar ao objeto `data`, preservando as **113 entradas existentes** intactas. Bump do campo `version` de `"2.0"` para `"3.0"`. Total final: 171 entradas.

**Contagem por categoria: 23 + 14 + 7 + 14 = 58 entradas novas.**
...
```json
"sales ops": "Analista de Sales Operations",
"revenue operations": "Analista de Sales Operations",
"revops": "Analista de Sales Operations",
...
"sdr": "Representante de Pré-Venda",
"bdr": "Representante de Pré-Venda",
"sales development representative": "Representante de Pré-Venda",
"business development representative": "Representante de Pré-Venda"
```

**Débito conhecido em `sdr`/`bdr`:** diferença semântica real (SDR = inbound, BDR = outbound) em algumas empresas. Catálogo atual tem apenas `Representante de Pré-Venda`. Word boundary previne falsos positivos em telecom (SDR = Software Defined Radio).
 ...
 * restrita a lista explícita de cargos conhecidos. Troca da regra "≤ 2 tokens"
 * por whitelist elimina falso positivo crítico: "Analista Sênior" vs "Analista Júnior"
 * (ambos 2 tokens, overlap 1 em "analista") estava passando com true, o que
 * permitiria Camada 0 clonar cross-seniority quando empresas usam templates
 * de descrição (Stone, BTG, Itaú têm Sênior/Pleno/Júnior com descrições idênticas).
 */
function titleGuardPasses(candidateTitle: string, anchorTitle: string): boolean {
...
**Implicação operacional:** ao editar `domain_synonyms.json`, o autor DEVE escrever as keys como a saída dessa função. Para as 58 entradas novas do patch §6.1, isso já foi validado: `"head of fp&a"` vira `"head of fp and a"` na normalização, então a key precisa estar escrita como `"head of fp&a"` no JSON (a transformação `&` → ` and ` ocorre no runtime). Mais simples: keys no JSON podem manter `&` original porque a normalização converte runtime. **O que não pode**: key com acento, com caixa mista, ou com barras/hífens — porque essas formas não sobrevivem à normalização.
...
async function main() {
    ...
    while (hasMore) {
        ...
        let query = supabase
            ...
            .lt('curated_at', DEPLOY_CUTOFF_TIMESTAMP)
        ...
        for (const job of batch) {
            try {
                ...
                if (job.curated_at && new Date(job.curated_at).toISOString() >= DEPLOY_CUTOFF_TIMESTAMP) {
...
if (!canonicalRole) {
    ...
    const { error } = await supabase
        ...
        .update({
            ...
            description_hash: options.descriptionHash ?? null,
            prompt_structure_version: options.promptStructureVersion ?? PROMPT_STRUCTURE_VERSION,
            prompt_content_version: options.promptContentVersion ?? PROMPT_CONTENT_VERSION,
            layer_2_hint_count: options.layer2HintCount ?? null,
    ...
    await supabase.from('events').insert({
        ...
        metadata: {
            ...
            prompt_structure_version: options.promptStructureVersion
        }
    });
...
export async function POST(
    ...
    // Busca estado atual para registrar previous_state
    ...
    // Se tentando revogar e já está false, no-op
...
**Políticas de revogação automática (opcional, v5.1 backlog):**

Quando admin faz `manual_remap` em uma vaga (muda canônico manualmente via endpoint existente), se `human_validated === true`, automaticamente setar para `false` — o remapeamento invalida a validação anterior. Fica como item backlog para evitar propagação de confiança obsoleta.
...
### 7.15 Trigger PostgreSQL: revogação de `human_validated` após `manual_remap` (v5.2 — N4)

**Motivação:** vaga marcada `human_validated=true` serve como âncora confiável na Camada 0. Se admin depois altera manualmente o `canonical_role_id` dessa vaga (via ação de `manual_remap` no admin), a validação anterior deixou de ser autoritativa — o canônico mudou, mas a flag `human_validated` continuava ativa, contaminando a Camada 0 com dados inconsistentes.

**Arquivo:** `docs/migrations/20260423_05_human_validated_revoke_trigger.sql`

```sql
-- ============================================================================
-- v5.2 — N4: revogação automática de human_validated após manual_remap
--
-- Quando admin altera canonical_role_id com curation_source='manual_remap',
-- a flag human_validated é revogada automaticamente. A validação humana
-- anterior foi feita para o canônico antigo; sobre o novo, não houve
-- decisão equivalente ainda.
-- ============================================================================

CREATE OR REPLACE FUNCTION revoke_human_validated_on_manual_remap()
RETURNS TRIGGER AS $$
BEGIN
    -- Só atua quando: (a) canonical_role_id mudou E (b) curation_source é 'manual_remap'
    IF NEW.canonical_role_id IS DISTINCT FROM OLD.canonical_role_id
    ...
    THEN
        ...
        INSERT INTO events (
            ...
            'human_validated_revoked_by_manual_remap',
            'job_posting',
            NEW.id,
            'system',
            '00000000-0000-0000-0000-000000000001',
            format('Validação humana revogada automaticamente após manual_remap de canônico (%s → %s)',
                   OLD.canonical_role_id, NEW.canonical_role_id),
            jsonb_build_object(
                'old_canonical_role_id', OLD.canonical_role_id,
                'new_canonical_role_id', NEW.canonical_role_id,
                'curation_source', NEW.curation_source
            )
        );
    END IF;
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trigger_revoke_human_validated_before_update
ON job_postings;

CREATE TRIGGER trigger_revoke_human_validated_before_update
BEFORE UPDATE ON job_postings
FOR EACH ROW
EXECUTE FUNCTION revoke_human_validated_on_manual_remap();

COMMENT ON TRIGGER trigger_revoke_human_validated_before_update
ON job_postings IS
    'v5.2 — N4: revoga human_validated quando manual_remap altera canonical_role_id.';
```

**Notas operacionais:**

- **Trigger `BEFORE UPDATE`:** permite modificar `NEW.human_validated` antes da escrita. Um trigger AFTER precisaria de UPDATE interno adicional, criando recursão.
- **Escopo estrito:** trigger só dispara se as 3 condições são verdadeiras (canonical mudou + curation_source='manual_remap' + human_validated era true). Outras formas de mudança de canonical_role_id (ex: merge de canônicos) não são afetadas — merges seguem regra diferente (veja §8.6).
- **Endpoint admin:** quando admin faz remap manual via UI, o backend DEVE setar `curation_source='manual_remap'` no UPDATE. Sem isso, trigger não dispara e flag fica indevidamente ativa. Item de validação no PR2.
- **Revogação para `false` (não NULL):** mantém o campo booleano não-nulo. Re-validação futura pelo admin restaura para `true` explicitamente.
...
2. Aplicar patch em `domain_synonyms.json` (v2.0 → v3.0 com 58 entradas; total final: 171)
...
10. Validação imediata: disparar curadoria manual de 50 vagas pending aleatórias via admin
11. Executar validação descrita em §9

**Aguardar 7 dias** em produção com PR1 antes de avançar para PR2.

**Fase 2 — PR2 (Infraestrutura pre-LLM) — ordem explícita obrigatória:**
...
5. Entre esses, conta quantos têm `canonical_role_id` divergente

**Critério de alerta:** se `divergentes_em_jaccard_alto / total_pares_jaccard_alto > 5%` por 2 semanas consecutivas, reabrir discussão de adicionar MinHash como Camada 0.5 (entre 0 e 1). Item fica como backlog observável, não como débito cego.
...
- [ ] Contagem: `jq '.data | length'` retorna 171 (113 + 58).
...
- [ ] **v5.2 — Teste `tests/domain-synonyms-hygiene.test.ts` (N1):** todas as 171 keys sobrevivem a `normalizeTitle` sem perder mais de 50% dos caracteres; não há duplicatas após normalização; todos os labels são strings não-vazias.
...
- [ ] **v5.2 — Teste de sanidade Camada 1 (6 casos — B1):** vagas com títulos específicos que DEVEM bater no `domain_synonyms.json` após correção de normalização de keys:
    - "Head of Finance" → `Diretor de Finanças`
    - "Sales Operations Manager" → `Analista de Sales Operations`
    - "Customer Service Representative" → `Analista de Customer Service`
    - "SDR" → `Representante de Pré-Venda`
    - "Tax Analyst" → `Analista Tributário`
    - **"Head of FP&A" → `Diretor de Finanças`** (caso crítico de `&` — teste que detecta B1 não corrigido)
...
**Agendamento do cron `/api/cron/curate-job-postings` em `vercel.json`.** Registro de cron é independente das mudanças desta spec.
...
- [ ] §2.1 — Colunas pré-requisitas presentes em produção (validação SQL via `information_schema.columns` retorna 5 linhas em `job_postings`, 1 em `analyses`, 1 em `ai_usage_logs`)
...
- [ ] §2.5 — Todos os itens de confirmação de estado reportados
- [ ] **v5.4 pós-pt.15 — Plano Vercel atual é Pro (não Hobby):** conferir via dashboard Vercel que o projeto está em plano Pro. O handler do `curate-job-postings` declara `maxDuration = 300s`, mas o plano Hobby tem cap de 10s — cron real falha. Se estiver em Hobby, fazer upgrade antes do deploy do PR2 para evitar timeouts silenciosos em produção
...
- [ ] Aplicar patch em `domain_synonyms.json` com bump 2.0 → 3.0 e 58 entradas novas
- [ ] Validar contagem final: 113 + 58 = 171
...
Executar APENAS após 7 dias de PR1 em produção sem regressão detectada.

**Passo 12 — Migração SQL (não tocar em código ainda):**
- [ ] **v5.4 — Backup pré-migration (§5.0):** executar os 4 `CREATE TABLE ... _backup_v5_4 AS SELECT *` para `job_postings`, `job_canonical_roles`, `skill_merge_decisions`, `role_merge_decisions` ANTES de qualquer ALTER TABLE. Validar via DO block que contagens batem nas 4 tabelas. Sem plano pago do Supabase, este é o único safety net para rollback operacional
- [ ] Aplicar migração §5.1 (10 colunas novas em `job_postings`, incluindo `canonical_role_label`, `layer_2_hint_count`, `canonical_resolved_at`, **`original_title`, `original_description` — v5.4**)
- [ ] **v5.4:** validar que backfill inline da §5.1 populou `original_title` e `original_description` em 100% das 8.308 vagas existentes: `SELECT COUNT(*) FROM job_postings WHERE original_title IS NULL OR original_description IS NULL` retorna 0
- [ ] **v5.4 pós-pt.15 — Aplicar migração §5.5:** `ALTER TABLE job_canonical_roles ADD COLUMN prompt_version text` + `UPDATE ... SET prompt_version = 'legacy'` + `ALTER COLUMN prompt_version SET NOT NULL DEFAULT 'unspecified'`. Validar: `SELECT COUNT(*) FROM job_canonical_roles WHERE prompt_version IS NULL` retorna 0. Validar: `SELECT COUNT(*) FROM job_canonical_roles WHERE prompt_version = 'legacy'` retorna igual ao total de canônicos pré-migration
- [ ] Aplicar migração §5.2 (3 índices: `idx_job_postings_description_hash_precheck`, `idx_job_postings_human_validated`, `idx_job_postings_curation_source`)
- [ ] Aplicar migração §5.4 (tabela `allowed_for_pre_resolution`)
- [ ] **v5.2 — Aplicar `docs/migrations/20260423_04_canonical_role_label_sync_trigger.sql` (§7.14, N3):** trigger de sincronia `canonical_role_label`
- [ ] **v5.2 — Aplicar `docs/migrations/20260423_05_human_validated_revoke_trigger.sql` (§7.15, N4):** trigger de revogação `human_validated` após `manual_remap`
- [ ] Popular whitelist com as decisões do Onsly obtidas em §2.4 (INSERTs para cada canônico aprovado opção b)
- [ ] Marcar `human_validated=true` em vagas manualmente validadas (Canonical GSI, Stone, AgileEngine — escopo do Onsly)
- [ ] Validar todas as colunas via `information_schema.columns`
- [ ] **v5.2 — Validar triggers:** `SELECT tgname FROM pg_trigger WHERE tgname IN ('trigger_sync_canonical_role_label_after_update', 'trigger_revoke_human_validated_before_update')` retorna 2 linhas
...
- [ ] **v5.4 (GS-R6-6) — Gate de pool inicial para rollout:** rodar query abaixo e confirmar que o pool efetivo de âncoras elegíveis é **≥ 50**. Se for menor, não subir rollout para 10% — fazer curadoria manual adicional de vagas de empresas com templates repetitivos (Canonical GSI, Stone, AgileEngine, Nubank, iFood) até o pool atingir o mínimo:
...
-- Gate:
--   >= 80: ótimo (pode subir para 10% com confiança)
--   50-79: aceitável (Camada 0 vai operar em observação, gatilhos só a partir de 25%)
--   < 50: BLOQUEAR rollout — fazer curadoria manual adicional primeiro
```
...
**Canônicos do patch — classificação apenas após execução da query §2.4.** Exemplos conhecidos da v3 (podem ter mudado desde então):
...
| Analista de Sales Operations | pending | (b) whitelist |
...
| Gerente de Finanças | não retornado | criar via upsert |
...
| Representante de Pré-Venda | pending | (b) whitelist |
...
## Anexo B — Decisões §2.4 cravadas (v5.4 final)
...
| # | Operação | Detalhes |
|---|---|---|
| 1 | MERGE | `Representante de Pré-Venda` (UUID `d780aacf-be11-4aeb-ad61-7b242a60178a`, 189 vagas) → `SDR` (UUID já `active`, 249 vagas). Resultado: SDR absorve, fica com 438 vagas. "Representante de Pré-Venda" vira `archived_at IS NOT NULL`. |
...
| 4 | ARCHIVE | `Supervisor de Pré-Vendas` (UUID `96a478e3-e5ca-4161-a624-450f547c7a36`, 0 vagas). Canônico zumbi. `archived_at` direto. |
...
1. Backup §5.0 (CREATE TABLE AS SELECT das 4 tabelas)
2. Migration §5.1 (colunas + backfill original_title/description)
3. **Migration §5.5 (coluna `prompt_version` em `job_canonical_roles` + backfill 'legacy')** — v5.4 pós-pt.15
4. Migration §5.2 (índices)
5. Migration §5.4 (tabela `allowed_for_pre_resolution`)
6. Migration dos triggers (§7.14, §7.15)
7. Seed C-level (CTO, CEO, COO, CMO, CFO — 5 INSERTs idempotentes, com `prompt_version = PROMPT_STRUCTURE_VERSION`)
8. Seed "Gerente de Finanças" como `active` (com `prompt_version = PROMPT_STRUCTURE_VERSION`)
9. Operações de taxonomia via tela admin: merges, renames, archive (5 operações listadas acima). Renames preservam `prompt_version` existente; merges preservam do target.
10. INSERTs na whitelist `allowed_for_pre_resolution` para canônicos (a) + (b) = 17 INSERTs (14 a + 4 b − 1 descontando Gerente de Finanças que já é active, se a política de seleção não cobrir; conferir política)
11. `human_validated = true` em vagas de pool inicial (Canonical GSI, Stone, AgileEngine)
12. Validações de pré-rollout: convergência (GS-R6-14), pool inicial (GS-R6-6)

Claude Code segue esta ordem exata no Passo 12. Se algo falhar, rollback via §5.0 + correção + re-execução a partir do passo que falhou.
