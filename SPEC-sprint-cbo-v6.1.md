# SPEC-sprint-cbo-v6.1

**Versão:** 6.1
**Data:** 06/05/2026
**Autor:** Onsly (PO) e Claude (arquiteto)
**Implementação:** Antigravity (TS) e Claude Code (SQL)
**Origem:** consolidação da v6 + 2 rodadas de validação externa (DeepSeek, GenSpark, Gemini, ChatGPT, Claude Code, Manus, Copilot, Mistral, Grok) + investigação empírica do Claude Code via 3 prompts dedicados sobre questões críticas em §2.7. Achado dominante: a §2.7 da v6 estava com transcrição significativamente incompleta vs body literal da v16 deployada. v6.1 §2.7 reescrita do zero baseada na extração literal de 137 linhas via `pg_get_functiondef` rodado contra produção em 06/05/2026. Correções adicionais: 7 ajustes técnicos acatados das rodadas externas, 2 LK-CBO novas, 1 Parte adicional (patch para v16 spec text alinhar D20 e §4.5.2 com a implementação real, conforme item 3 priorizado pelo PO).

---

# Changelog v6 → v6.1

## 🚨 Bloqueador descoberto pela investigação Claude Code

| Item | Origem | Correção |
|---|---|---|
| **§2.7 transcrição incompleta** | Investigação Claude Code (Prompt 2) | §2.7 reescrita verbatim do body literal de 137 linhas extraído via `pg_get_functiondef` em produção. Correções: (a) nome real do evento `canonical_promoted_dynamic` (não `canonical_promoted_on_threshold` como cravado na v6), (b) branch ELSE preservado com UPDATE + INSERT de `canonical_promotion_deferred_archaeological` + 3 EXCEPTION handlers, (c) EXCEPTION aninhado completo do `auto_assign_family_to_canonical` com fallback INSERT em events, (d) metadata rico com `promotion_rule`, `thresholds`, `has_recent_vacancy`, `trigger_type` |

## 🟠 Ajustes técnicos das rodadas externas

| Item | Origem | Correção |
|---|---|---|
| V6-O1 / Gemini #1 / ChatGPT B2 (TRIPLICATA) | Race em §2.11 INSERT slug livre | `BEGIN ... EXCEPTION WHEN unique_violation THEN ...` redirecionando para sufixação. PostgreSQL Read Committed não tem gap locks; `SELECT FOR UPDATE` em row inexistente não bloqueia INSERT concorrente |
| V6-O2 | resource_type inconsistente em event opus_replaced_cbo_link | Trocado para `'job_canonical_role'` + info do link movida para metadata, alinhado com demais events de mutação canonical |
| V6-O4 | §2.12 sem guard simétrico contra placeholder CBO | RAISE EXCEPTION se `source='cbo_mte_2002_seed'` no canonical alvo, simétrico ao guard de §2.11 V5-M5 |
| Gemini #3 | OOM em §3.7 detectStaleCanonicals sem `.limit()` | `.limit(500)` adicionado, margem ~50x acima do cap MAX_ALERTS=10 |
| ChatGPT B3 | retire_canonical sem revalidar `vacancy_count=0` no UPDATE | UPDATE ganha cláusula `(p_reason='zero_vacancy_count' AND vacancy_count=0) OR (p_reason='no_recent_postings_365d' AND NOT EXISTS ...)`, defesa simétrica para os 2 reasons |
| ChatGPT M1 | createFamily §3.3 sem proteção contra race | `BEGIN ... EXCEPTION WHEN unique_violation THEN SELECT id ...`, mesmo padrão de §2.11 |
| Manus 1 (revisitado pós-investigação) | Ressuscitação via §2.11 ON CONFLICT sem força revisão Opus | `UPDATE ... SET needs_opus_review=TRUE` no branch de ressuscitação, simétrico ao branch ELSE da v16 deployada que já marca canônicos pending para revisão Opus |
| ChatGPT polish | Rodapé "Fim da SPEC-sprint-cbo-v5" | Atualizado para "Fim da SPEC-sprint-cbo-v6.1" |

## 📚 Limitações conhecidas novas/atualizadas

| Item | Origem | Conteúdo |
|---|---|---|
| LK-CBO-19 (atualizada) | Investigação Claude Code (Prompt 1) | Decisão arquitetural opção (a) preservar pattern AFTER + `pg_trigger_depth() > 1` + UPDATE explícito da v16 deployada. Lição metodológica: ground truth é `schema.sql` extraído via `pg_get_functiondef` da live Supabase, não spec text histórica |
| LK-CBO-20 (NOVA) | Investigação Claude Code (Prompt 3) | Design intencional do guard `pg_trigger_depth() > 1`. Bloqueia cascata via INSERT em job_postings (depth=2) por escolha arquitetural. Promoção legítima vem do CRON `pipeline-maintenance` via função `catchup_pending_promotions()` que faz "+1/-1 vacancy_count" para forçar trigger em depth=1. Razão: separar evento "vaga chega" de "decisão de promoção" para evitar race com `auto_assign_family_to_canonical`. Evidência empírica: 31 promoções canonical_promoted_dynamic em 90d via catchup, 0 promoções via cascata |
| LK-CBO-21 (NOVA) | Lição metodológica desta sprint | Transcrição literal de body de função em sessões compactadas tem risco de perda de detalhes. Protocolo definido para próximas sprints: extrair body via `pg_get_functiondef` direto antes de cravar em spec, validar via diff com `schema.sql` deployado. v6 §2.7 foi simplificada por engano (perda do branch ELSE, EXCEPTION blocks, nome do evento). v6.1 corrige verbatim |
| LK-CBO-22 (NOVA) | Investigação Claude Code (Prompt 1) | Drift documental entre v16 spec text (D20 e §4.5.2 documentam gate `confidence_median >= 0.75`) e v16 deployada (gate nunca foi implementado, body real só usa `confidence_median` para snapshot em `confidence_median_at_promotion`). Patch para v16 spec text incluído na Parte 10 desta SPEC alinhando documentação à realidade |

## 🆕 Parte 10 — Patch v16 spec text

Item 3 priorizado pelo PO. Sub-PR adicional na sprint CBO atualiza D20 e §4.5.2 da SPEC-sprint-v16 removendo referências ao gate `confidence_median >= 0.75` que nunca foi implementado em produção. Redação literal do patch delegada ao Claude Code após v6.1 fechada (será executada via `view` do arquivo v16 + `str_replace` cirúrgico nas 2 zonas afetadas).

## 📋 Itens rejeitados nesta rodada (com justificativa)

| Item | Origem | Justificativa |
|---|---|---|
| ChatGPT M2 (gate confidence_median em §2.7) | ChatGPT | Investigação Claude Code (Prompt 1) confirmou: gate não existe na v16 deployada. ChatGPT confundiu spec text com implementação. v6 §2.7 estava correta nesse ponto |
| ChatGPT B1 (whitelist do monitor de promoções órfãs) | ChatGPT | Investigação Claude Code (Prompt 2) confirmou: monitor não existe (nem em código, nem em pg_proc, nem em CRON). Premissa do revisor falsa. Sem ação operacional decorrente |
| Gemini #2 (pg_trigger_depth como bug) | Gemini | Investigação Claude Code (Prompt 3) confirmou empiricamente: guard é design intencional, não bug. catchup_pending_promotions é o caminho oficial. Documentado em LK-CBO-20 |
| Manus 2 (replace_cbo_link sem check final de primary) | Manus | Argumento técnico errado. Função PL/pgSQL é executada em transação; falha de INSERT causa rollback automático da transação inteira. Cenário descrito não pode ocorrer |
| Manus 3 (starvation por ORDER BY) | Manus | Falsa premissa. Registros que recebem vagas têm MAX(posted_at) recente, por definição não são stale. ORDER BY ASC traz primeiro candidatos mais prováveis de serem stale |
| Manus 4 (flag silent para events) | Manus | Conflita com requisito de auditoria forense (D-CBO-29). Events são audit log por design |
| Copilot inteira (reduzir escopo MVP) | Copilot | Não levantou bug técnico. Sugestões desfazem decisões arquiteturais já tomadas (D-CBO-26, D-CBO-29, D-CBO-31). Audit de escopo não é audit técnica |
| Mistral inteira | Mistral | "Não consegue acessar o conteúdo" pela 3ª vez consecutiva. Inutilizável |
| Grok inteira | Grok | Revisou outra sprint (sprint v14, não CBO). Sub-PR14/17, D2, S6/S9/S10 não existem nesta sprint |

---

# Changelog v5 → v6 (preservado para histórico)
...
ALTER do trigger Frente A baseado no **body literal completo da v16 deployada** extraído via `pg_get_functiondef` rodado contra produção em 06/05/2026 (137 linhas, 6.303 caracteres). **Crítico:** v6 cravou versão simplificada com 3 perdas significativas: (1) nome errado do evento (`canonical_promoted_on_threshold` em vez do real `canonical_promoted_dynamic`), (2) branch ELSE ausente (que marca `needs_opus_review=TRUE` quando gates 1+2 cruzam sem vagas recentes), (3) EXCEPTION blocks ausentes. v6.1 corrige verbatim. **Mudança CBO mínima:** estender 2 IFs para `('pending', 'retired')`, COALESCE em campos snapshot, flag `is_resurrection` no metadata.
...
DECLARE
    ...
    v_is_resurrection BOOLEAN := FALSE;  -- EXTENSÃO CBO: distinguir promoção inicial de ressuscitação
BEGIN
    -- Recursion guard: design intencional da v16 deployada (LK-CBO-20).
    ...
    -- EXTENSÃO CBO: estender enum status para suportar ressuscitação retired→active (D-CBO-24).
    -- Resto do body preservado verbatim da v16 deployada (schema.sql:3473-3608, 137 linhas).
    ...
    THEN
        ...
        IF v_has_recent_vacancy THEN
            ...
            BEGIN
                INSERT INTO events (
                    ...
                    'canonical_promoted_dynamic',  -- preservado verbatim da v16 (rastreabilidade dos 31 events em 90d)
                    'system',
                    ...
                    jsonb_build_object(
                        ...
                        'is_resurrection', v_is_resurrection  -- EXTENSÃO CBO: distingue promoção inicial de ressuscitação
                    )
                );
            EXCEPTION
        ...
        ELSE
            ...
            -- Branch preservado verbatim da v16 deployada. Cold path: zero ocorrências
            -- em produção em 90d (cenário exige 3+ vagas E nenhuma ativa nos últimos
            -- 60d — vagas antigas tendem a ter is_active=false).
                ...
                BEGIN
                    INSERT INTO events (
                        ...
                        jsonb_build_object(
                            ...
                            'previous_status', OLD.status  -- EXTENSÃO CBO: pode ser 'pending' OU 'retired'
                        )
                    );
                EXCEPTION
                    WHEN foreign_key_violation THEN
...
- **Trigger registration preservado da v16:** `CREATE TRIGGER trg_promote_on_threshold AFTER UPDATE OF vacancy_count, distinct_sources_count ON public.job_canonical_roles FOR EACH ROW EXECUTE FUNCTION fn_promote_canonical_on_threshold()`. Nome do trigger é `trg_promote_on_threshold`, sem prefixo `z_`. Não alterar.
- **Coluna `needs_opus_review`:** tipo `boolean NOT NULL DEFAULT false` em `job_canonical_roles`. Predicado `IS DISTINCT FROM TRUE` é equivalente a `= FALSE` em produção, mas preservado verbatim como defesa em profundidade contra mudança futura para nullable.
- **3 triggers UPDATE coexistentes em `job_canonical_roles`** (todos enabled): `trg_promote_on_threshold` (este), `auto_assign_family_on_active` (AFTER UPDATE OF status, embedding com WHEN new.status='active'), `trg_reset_embedding_on_semantic_change` (BEFORE UPDATE). Quando este trigger faz UPDATE SET status='active', dispara cascade em `auto_assign_family_on_active` que executa `trigger_auto_assign_family` (provável wrapper de `auto_assign_family_to_canonical`). Como este trigger TAMBÉM faz `PERFORM auto_assign_family_to_canonical` direto, a função pode ser chamada duas vezes na mesma transação. Comportamento herdado da v16, fora do escopo da extensão CBO.
- **Branch ELSE com `canonical_promotion_deferred_archaeological`:** caminho real do código (não dead code), mas exercido zero vezes em produção em 90d. Preservado verbatim porque cenário pode emergir após Sprint CBO quando placeholders CBO ressuscitarem com volume mas sem vagas recentes.
- **`is_resurrection` no metadata:** única extensão CBO pura no payload do event. Permite distinguir promoção inicial de ressuscitação em queries de auditoria sem afetar rastreabilidade dos 31 events legados.
...
BEGIN
    ...
    -- V5-M4 + ChatGPT B3 (v6.1): UPDATE com revalidação SIMÉTRICA dos 2 reasons.
...
BEGIN
  ...
  IF v_existing_slug_canonical_id IS NULL THEN
    ...
    -- V6-O1 / Gemini #1 / ChatGPT B2 (v6.1, TRIPLICATA): PostgreSQL Read Committed
  ...
  ELSIF v_existing_slug_status = 'retired' THEN
    ...
    -- v6.1 (Manus 1 simetria): força needs_opus_review=TRUE no canônico ressuscitado,
    -- alinhando com pattern do branch ELSE da v16 deployada que marca canônicos
...
BEGIN
  ...
  -- V6-O4 (v6.1): guard simétrico ao §2.11 V5-M5.
  ...
  -- V6-O2 (v6.1): resource_type consistente com demais events de mutação canonical.
  -- Antes (v6): resource_type='canonical_role_cbo_link' divergente.
  -- Agora (v6.1): resource_type='job_canonical_role' alinhado.
  -- Info do link movida para metadata.
  INSERT INTO events (event_name, actor, actor_id, resource_type, resource_id, metadata)
  VALUES (
...
async function createFamily(
  ...
  // ChatGPT M1 (v6.1): proteção contra race condition.
  // Mesmo padrão de §2.11: PostgreSQL Read Committed não tem gap locks; duas
  // chamadas concorrentes de findOrCreateFamily com mesma família nova podem
  // ambas chegar até aqui e ambas tentar INSERT. Captura unique violation e
  // retorna a row vencedora (criada pela outra instância).
  try {
    ...
    if (err?.code === '23505' || err?.message?.includes('duplicate key')) {
      ...
      if (existing) {
        ...
        return { family_id: existing.id, outcome: 'create_new' };
      }
    }
    throw err;
  }
...
export async function detectStaleCanonicals(
  ...
  // Gemini #3 (v6.1): .limit(500) para evitar OOM em serverless Vercel.
  // Sistema maduro pode ter dezenas de milhares de canonicals active+vacancy>0;
  // carregar tudo estoura heap (~500MB-1GB). Como MAX_ALERTS_PER_EXECUTION=10,
  // 500 dá margem ~50x acima do cap.
...
| LK-CBO-19 | §2.7 preserva pattern AFTER UPDATE + `pg_trigger_depth()` + UPDATE explícito da v16 deployada extraído via `pg_get_functiondef` em produção. Decisão arquitetural opção (a) priorizando estabilidade vs opção (b) que reverteria para BEFORE+NEW assignments. Justificativa: preservar pattern em produção minimiza risco de regressão e mantém compatibilidade com `catchup_pending_promotions()` (LK-CBO-20). Lição metodológica cravada: ground truth para body de função é `schema.sql` extraído de live Supabase, não spec text histórica nem transcrições compactadas (LK-CBO-21) |
...
| LK-CBO-21 | Lição metodológica: transcrição literal de body de função PL/pgSQL em sessões compactadas tem risco de simplificação por engano. v6 §2.7 perdeu 3 elementos críticos da v16 deployada: (a) nome real do evento `canonical_promoted_dynamic` virou `canonical_promoted_on_threshold` (quebraria rastreabilidade dos 31 events em 90d), (b) branch ELSE inteiro com UPDATE `needs_opus_review=TRUE` + INSERT `canonical_promotion_deferred_archaeological` desapareceu, (c) 3 EXCEPTION blocks com handlers para `foreign_key_violation`, `unique_violation`, `OTHERS` foram trocados por `PERFORM` direto. v6.1 corrigiu via extração literal `pg_get_functiondef` rodada contra produção. Protocolo para próximas sprints: extrair body via `pg_get_functiondef` direto antes de cravar em spec, validar via diff com schema.sql deployado |
| LK-CBO-22 | Drift documental v16: spec text declara em D20 e §4.5.2 o gate `confidence_median >= 0.75` como condição de promoção, mas a v16 deployada nunca implementou esse gate. Body real de `fn_promote_canonical_on_threshold` usa `confidence_median` apenas para snapshot em `confidence_median_at_promotion`, sem filtro. Patch para alinhar v16 spec text à implementação está incluído na Parte 10 desta SPEC, redação literal delegada ao Claude Code |
...
# Parte 10 — Patch para v16 spec text (drift documental LK-CBO-22)

Item 3 priorizado pelo PO: alinhar a documentação histórica da v16 (`SPEC-sprint-v16.md`) à implementação real em produção. Investigação Claude Code (Prompt 1, 06/05/2026) confirmou que o gate `confidence_median >= 0.75` documentado em D20 e §4.5.2 da v16 spec text **nunca foi implementado** no body real de `fn_promote_canonical_on_threshold`.
...
- Conteúdo alvo: remover `, confidence_median ≥ 0.75`. Mantém `"Gate de promoção: ≥3 vagas, ≥2 fontes, janela 60d."`. Adicionar nota de rodapé referenciando LK-CBO-22 da SPEC-sprint-cbo-v6.1.
...
- Conteúdo alvo: remover essa linha do body documentado. Adicionar comentário acima do bloco SQL: `-- Nota: gate confidence_median nunca foi implementado em produção. Body real só usa confidence_median para snapshot em confidence_median_at_promotion. Vide LK-CBO-22 da SPEC-sprint-cbo-v6.1.`

## 10.2 — Execução

Delegar redação literal ao Claude Code após v6.1 fechada. Sequência:

1. Claude Code faz `view` em `SPEC-sprint-v16.md` linhas 130-150 e 1690-1720 para confirmar conteúdo exato vs estimado acima
2. Claude Code aplica `str_replace` cirúrgico nas 2 zonas com diff mínimo
3. Claude Code adiciona linha de changelog no topo do arquivo: `"06/05/2026 - Patch documental aplicado (sprint CBO §10.1): removida referência ao gate confidence_median >= 0.75 em D20 e §4.5.2. Gate nunca foi implementado em produção. Vide LK-CBO-22."`
4. Claude Code reporta diff aplicado para validação do PO
...
- Linha de changelog do topo do arquivo inclui referência ao patch

## 10.4 — Por que esse patch entra na sprint CBO

Decisão do PO. A descoberta do drift veio da investigação do Prompt 1 (Claude Code) feita para validar §2.7 da v6.1. Inclusão na mesma sprint evita ticket separado de manutenção e mantém a trilha de auditoria coesa: investigação → descoberta → correção, tudo sob a sprint que descobriu o problema.

---

**Fim da SPEC-sprint-cbo-v6.1.**
