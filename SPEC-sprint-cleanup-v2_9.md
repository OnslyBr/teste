# SPEC — Sprint Cleanup v2.9

**Projeto:** CalibraCV
**Data de fechamento da spec:** 2026-05-13
**Versão:** cleanup-v2.9
**MER de referência:** v37
**Versão anterior:** cleanup-v2.8

### Correções acumuladas v2.7 → v2.9 (ground truth via DB ao vivo)

| ID | Versão | Severidade | Local | Correção |
|---|---|---|---|---|
| **C-1** | v2.8 | CRÍTICA | §2.1 | Constraint name corrigido: `jcs_source_check` → `job_canonical_skills_source_check` (nome real no DB). Source values corrigidos: `job_posting_skill` não existe — valores reais são `llm_extractor`, `cbo_mte_2002_seed`, `manual_admin`, `opus_arbitration`. Adicionado `resume_extraction`. |
| **H-1** | v2.8 | ALTA | §2.2 | Constraint name corrigido: `jcr_source_check` → `job_canonical_roles_source_check` (nome real no DB). Adicionados `seed` (estava no constraint original), `job_posting`, `resume_extraction`, `manual_admin`, `opus_arbitration`. |
| **H-3** | v2.8 | ALTA | §2.20 | `DROP TRIGGER IF EXISTS trg_pipeline_config_audit ON pipeline_config` adicionado antes da função. Sem isso, cada UPDATE produzia 2 entradas em `pipeline_config_history` (trigger legado + INSERT explícito). Trigger confirmado existente via DB. |
| **C-3** | v2.8 | CRÍTICA | §2.7a (NOVA) | Migration nova `07a_resume_roles_confidence_type.sql` converte `resume_roles.confidence` de `smallint` (0-100) para `numeric(5,4)` (0.0000-1.0000). Sem isso, confidence LLM 0.75 era armazenado como `1` (smallint arredonda). CHECK antigo (0-100) substituído por CHECK (0-1). |
| **C-4** | v2.8 | CRÍTICA | §2.4 | WHERE clause corrigida: `lower(label) = lower(trim(p_label))` → `normalize_skill_label(label) = normalize_skill_label(p_label)`. Index UNIQUE em JCS usa `normalize_skill_label()` — versão anterior causava seq scan e falso negativo para C#/C++/skills acentuadas. |
| **C-4b** | v2.8 | CRÍTICA | §2.6 | WHERE clause corrigida: `lower(source_term) = lower(trim(p_term))` → `source_term = normalize_skill_label(p_term)`. Values em `taxonomy_relations.source_term` são pré-normalizados (sem acentos) — `lower(trim())` não stripa acentos e produziria falso negativo. `normalize_role_label` não existe no DB; reutilizar `normalize_skill_label` é correto. |
| **M-1** | v2.8 | MÉDIA | §3.5 | Instrução corrigida: remover menção a `canonicalRoleId` como 4º argumento. §3.2 explicitamente removeu esse parâmetro — instrução conflitante causaria erro de compilação TypeScript. |
| **F-1** | v2.8 | BAIXA | §3.3 | Comentário TS corrigido: `jcr_source_check` → `job_canonical_roles_source_check`. |
| **F-2** | v2.8 | BAIXA | §2.22 | Documentada arquitetura dois-triggers por entidade: BEFORE flag-setter (recriado) + AFTER `_emit` (preservado). Instrução explícita de não dropar `trg_flag_needs_opus_review_jcr_emit` / `_jcs_emit`. |
| **CR-1** | v2.9 | ALTA | §2.13 / §2.14 | `p_source` sem DEFAULT em `reconcile_canonical_skill` e `reconcile_canonical_role`. Decisão de produto: zero callers existentes (funções novas); todos os callsites em v2.9 passam `p_source` explicitamente; default 'llm_extractor' silenciaria erros de caller — fail-fast via constraint violation é preferível. |
...
5. **Sem novos métodos de comunicação ou controles operacionais.** Esta sprint não introduz SMS, e-mail, toast, push, banner, modal extra, webhook externo, budget guard, kill-switch, rate limiter, monitor, dashboard, ou alerta novo. O registry §2.17 (`admin_panel_functions`) é instrumentação de uso das telas admin existentes (não cria novos alertas ou notificações).
...
|---|---|---|---|
| §2.4 / §2.5 | `lookup_canonical_*_by_normalized_label` | ✓ par simétrico | 2 funções, 1 por entidade |
| §2.7 + §2.8 / §2.8b | `resume_roles` (ADD FK + flag) / `resume_skills` (tabela nova) | ✓ par simétrico | extração de currículo paralela; skills via nova tabela |
...
| §2.31 | `taxonomy_blacklist` | ✓ `entity_type CHECK ('role','skill')` | tabela única polimórfica |
| §2.32 / §2.33 / §2.34 | painéis 11, 12, 13 | ✓ role + skill via UNION ALL | jsonb com chaves `role:{}` + `skill:{}` |
| §2.36 | `reject_and_blacklist_canonical_pair` | ✓ par dispatcher por `p_entity_type` | 1 RPC, 2 branches |
| §3.2 / §3.3 | `extractAndPersistResumeSkills` / `_Roles` | ✓ par simétrico | mesma forma, INSERT em `resume_skills` / `resume_roles` |
...
Ordem de execução = ordem de numeração. Cada migration é um arquivo em `docs/migrations/sprint-cleanup-v2/NN_descricao.sql`. Todas com transação explícita (`BEGIN; ... COMMIT;`), `SET LOCAL statement_timeout` e snapshot prévio do estado das funções afetadas via `pg_get_functiondef`.

### §2.1 — CHECK constraint `job_canonical_skills_source_check` ampliado

**Arquivo:** `01_jcs_source_check.sql`
**Objetivo:** garantir que `job_canonical_skills.source` aceita todos os valores necessários para os fluxos atuais, incluindo `resume_extraction` (pipeline de CV) que estava faltando.

**Ground truth confirmado:** o constraint real no banco é `job_canonical_skills_source_check` (não `jcs_source_check`). Os 53.478 rows existentes têm `source = 'cbo_mte_2002_seed'`. O valor `resume_extraction` é necessário para o fluxo §3.2.

```sql
BEGIN;

-- Nome real do constraint confirmado via pg_constraint (não 'jcs_source_check')
ALTER TABLE job_canonical_skills DROP CONSTRAINT IF EXISTS job_canonical_skills_source_check;

ALTER TABLE job_canonical_skills ADD CONSTRAINT job_canonical_skills_source_check
CHECK (source IN (
  'llm_extractor',
  'cbo_mte_2002_seed',
  'manual_admin',
...
**Validação:** `SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname = 'job_canonical_skills_source_check';` deve listar os 5 valores incluindo `resume_extraction`.
...
**Ground truth confirmado:** o constraint real no banco é `job_canonical_roles_source_check` (não `jcr_source_check`). Os 2.694 rows existentes têm `source = 'cbo_mte_2002_seed'`. Constraint atual aceita `'seed', 'llm_extractor', 'cbo_mte_2002_seed'` — esta migration expande para incluir os valores necessários para os novos fluxos.

```sql
BEGIN;

-- Nome real do constraint confirmado via pg_constraint (não 'jcr_source_check')
ALTER TABLE job_canonical_roles DROP CONSTRAINT IF EXISTS job_canonical_roles_source_check;

ALTER TABLE job_canonical_roles ADD CONSTRAINT job_canonical_roles_source_check
CHECK (source IN (
  'seed',
  'llm_extractor',
  'cbo_mte_2002_seed',
...
`cbo_mte_2002_seed` e `seed` preservados (2.694 rows existentes usam `cbo_mte_2002_seed`; `seed` estava no constraint original e é mantido por segurança). `resume_extraction` necessário para §3.3 (pipeline de CV).
...
### §2.4 — `lookup_canonical_skill_by_normalized_label` aceita pending

**Arquivo:** `04_lookup_canonical_skill_by_normalized_label.sql`
**Objetivo:** permitir que o pipeline encontre canonicals `pending` quando recém-criados, evitando duplicatas. Usa `label` (não `canonical_label`) per schema real do JCS.

**Ground truth confirmado:** o índice UNIQUE em JCS é `uq_jcs_label_normalized ON job_canonical_skills USING btree (normalize_skill_label(label)) WHERE (status = 'active' AND merged_into IS NULL)`. A função de busca DEVE usar `normalize_skill_label()` — não `lower(trim())` — para (a) bater no índice e evitar seq scan, (b) encontrar corretamente canonicals cujos labels contêm símbolos especiais (`C#` → `csharp`, `C++` → `cpp`) ou acentos PT-BR. `lower(trim(p_label))` produziria falso negativo para esses casos.
...
BEGIN
  ...
  -- normalize_skill_label() é obrigatório aqui — coincide com a expressão do índice
  -- uq_jcs_label_normalized (btree normalize_skill_label(label) WHERE active/not merged).
  -- lower(trim()) produziria falso negativo para C# (stored 'csharp'), C++ (stored 'cpp'),
  -- e labels com acentos PT-BR.
  SELECT id INTO v_id
  FROM job_canonical_skills
  WHERE normalize_skill_label(label) = normalize_skill_label(p_label)
    AND status IN ('active', 'pending')
    AND merged_into IS NULL
    AND (p_employer_id IS NULL OR employer_id IS NULL OR employer_id = p_employer_id)
  ORDER BY
    CASE WHEN status = 'active' THEN 0 ELSE 1 END,
    CASE WHEN employer_id = p_employer_id THEN 0 ELSE 1 END,
    created_at ASC
  LIMIT 1;
...
### §2.5 — `lookup_canonical_role_by_normalized_label` condicional

**Arquivo:** `05_lookup_canonical_role_by_normalized_label.sql`
**Objetivo:** análogo a §2.4 para JCR. Usa `canonical_label` (campo real do JCR, assimetria intencional vs JCS conforme D-PS-08).
...
BEGIN
  ...
  WHERE lower(canonical_label) = lower(trim(p_canonical_label))
    AND status IN ('active', 'pending')
    AND merged_into IS NULL
    AND (p_employer_id IS NULL OR employer_id IS NULL OR employer_id = p_employer_id)
  ORDER BY
...
**Ground truth confirmado:** `taxonomy_relations.source_term` para roles contém valores pré-normalizados pelo CBO seed (sem acentos, lowercase, separadores como ` - ` viram espaço). Ex.: `"abastecedor de linha de producao"` (não `"abastecedor de linha de produção"`). `lower(trim(p_term))` não strip acentos e produziria falso negativo ao buscar "Abastecedor de Linha de Produção". A função `normalize_skill_label()` implementa exatamente a normalização que o seed usou (lowercase + strip acentos PT-BR + separadores → espaço). Não existe `normalize_role_label` no banco — reutilizar `normalize_skill_label()` é correto para roles (o SYMBOL_LOOKUP c++→cpp não interfere em labels de cargo).
...
BEGIN
  ...
  -- normalize_skill_label() é usado aqui porque não existe normalize_role_label no banco,
  -- e a normalização implementada (lowercase + strip acentos PT-BR + separadores → espaço)
  -- é a mesma aplicada ao gravar source_term no seed CBO. lower(trim()) não faria strip de acentos.
  SELECT target_role_id INTO v_id
  FROM taxonomy_relations
  WHERE source_term = normalize_skill_label(p_term)
    AND status = 'active'
...
### §2.7a — `resume_roles.confidence` migração de `smallint` para `numeric(5,4)` (NOVA)

**Arquivo:** `07a_resume_roles_confidence_type.sql`
**Objetivo:** corrigir o tipo da coluna `resume_roles.confidence` de `smallint` (0-100) para `numeric(5,4)` (0.0000-1.0000) para suportar os valores de confiança em float 0-1 produzidos pelo LLM e inseridos em §3.3. Sem esta migration, `confidence: 0.75` seria armazenado como `1` (arredondamento smallint) — perda total de precisão.

**Ground truth confirmado:** `resume_roles.confidence` é `smallint` com CHECK `(confidence >= 0 AND confidence <= 100)`. Base está vazia (pré-MVP sem usuários). A migration altera o tipo e substitui o CHECK por um compatível com a semântica float 0-1.

**Ordem crítica:** esta migration deve executar **antes** de §2.8 (que adiciona `passed_hard_gate` e um CHECK de consistência `confidence >= 0.70` — a comparação só faz sentido semântico com tipo numérico 0-1). Arquivo prefixado `07a` para garantir execução antes de `08`.

```sql
BEGIN;

-- Pré-MVP: tabela vazia, ALTER TYPE sem USING problemático de conversão de escala
ALTER TABLE resume_roles ALTER COLUMN confidence TYPE numeric(5,4)
  USING confidence::numeric(5,4);

-- Substitui CHECK antigo (0-100 para smallint) pelo CHECK correto para float 0-1
ALTER TABLE resume_roles DROP CONSTRAINT IF EXISTS chk_resume_roles_confidence;

ALTER TABLE resume_roles ADD CONSTRAINT chk_resume_roles_confidence
...
-- Esperado: data_type='numeric', numeric_precision=5, numeric_scale=4

SELECT pg_get_constraintdef(oid) FROM pg_constraint
WHERE conname = 'chk_resume_roles_confidence';
-- Esperado: CHECK ((confidence IS NULL OR (confidence >= 0 AND confidence <= 1)))
```

---

### §2.7 — `resume_roles ADD canonical_role_id` FK

**Arquivo:** `07_resume_roles_canonical_role_id.sql`
...
### §2.8 — `resume_roles ADD passed_hard_gate`
...
ALTER TABLE resume_roles
  ADD CONSTRAINT resume_roles_passed_hard_gate_consistency
  CHECK (
    NOT passed_hard_gate
    OR (confidence IS NOT NULL AND confidence >= 0.70)
  );
...
**Validação:** registros gravados com `passed_hard_gate = true` precisam ter `confidence >= 0.70` (float 0-1, conforme tipo `numeric(5,4)` estabelecido em §2.7a que deve executar antes desta migration). Atende sugestão Manus2 sobre CHECK constraint de consistência.

---

### §2.8b — `resume_skills` (tabela nova, espelha `resume_roles`)

**Arquivo:** `08b_resume_skills_table.sql`
**Atende:** simetria skill↔role na camada de extração do currículo. Ground truth confirmou que `analysis_skill_matches` é cache de match calculado (com `frequency`, `matched`, `contribution`, `match_status` todos NOT NULL) — não pode ser usada como store de skills brutas extraídas pós-hard-gate, sob pena de violação de NOT NULL em runtime. Criar tabela paralela seguindo exatamente o padrão de `resume_roles`.
...
CREATE TABLE IF NOT EXISTS resume_skills (
  ...
  CONSTRAINT resume_skills_passed_hard_gate_consistency
    CHECK (
      NOT passed_hard_gate
      OR (confidence IS NOT NULL AND confidence >= 0.70)
    )
...
-- UNIQUE INDEX funcional (espelha §2.25 para resume_roles)
CREATE UNIQUE INDEX IF NOT EXISTS uniq_resume_skills_resume_skill_normalized
  ON resume_skills (resume_id, lower(trim(skill)));
...
'Skills extraídas do currículo após hard gate, paralela a resume_roles. Distinto de analysis_skill_matches (cache de match calculado por análise contra função) e de resume_skill_enrichments (log de validação user-facing).';

COMMIT;
```

**Distinções importantes (evita confusão com tabelas vizinhas de skill):**
- `resume_skills` (esta) — skills brutas extraídas do currículo, pós-hard-gate, com `canonical_skill_id` resolvido. Análoga a `resume_roles`.
...
### §2.9 — Trigger `latest_posted_at` em JCR (primeira implementação)

**Arquivo:** `09_jcr_latest_posted_at_trigger.sql`
**Nota descritiva:** esta é a **primeira** implementação do padrão `latest_posted_at` em JCR (JCS usa cálculo embutido em `fn_jps_recompute_jcs`, sem trigger dedicado). Não é "paridade com JCS" — é introdução do mecanismo apenas no lado JCR onde fazia falta.
...
BEGIN
  ...
  SET latest_posted_at = GREATEST(
    COALESCE(latest_posted_at, '-infinity'::timestamptz),
    NEW.posted_at
  )
  WHERE id = NEW.canonical_role_id
...
AFTER INSERT OR UPDATE OF posted_at, canonical_role_id ON job_postings
FOR EACH ROW
WHEN (NEW.canonical_role_id IS NOT NULL AND NEW.posted_at IS NOT NULL)
...
6. Falha: emite evento `canonical_orphan_no_family_assigned`
...
BEGIN
  ...
  ELSE
    INSERT INTO events (
      ...
      'canonical_orphan_no_family_assigned',
      'job_canonical_role',
      p_role_id,
      'system',
      v_actor_id,
...
### §2.13 — `reconcile_canonical_role` (corrigida)

**Arquivo:** `13_reconcile_canonical_role.sql`
**Bloqueadores resolvidos:** A5 (schema events), B6 (`employer_id` não `employer_normalized_id`).

```sql
BEGIN;

-- CR-1: p_source sem DEFAULT — caller obrigatório. Fail-fast via CHECK constraint
-- é preferível a silenciar um caller esquecido com 'job_posting' incorreto.
-- Todos os callsites em v2.9 passam p_source explicitamente.
...
BEGIN
  ...
  INSERT INTO job_canonical_roles (
    canonical_label, status, source, employer_id, vacancy_count, distinct_sources_count
  ) VALUES (
    trim(p_canonical_label), 'pending', p_source, p_employer_id, 0, 0
  )
...
### §2.14 — `reconcile_canonical_skill` (corrigida)

**Arquivo:** `14_reconcile_canonical_skill.sql`
**Bloqueadores resolvidos:** A5, B6, A4 (sem filtro `curation_status='curated_fallback'` que era inacessível por CHECK conflict v11).

```sql
BEGIN;

-- CR-1: p_source sem DEFAULT — caller obrigatório. 'job_posting_skill' sequer existe
-- na constraint JCS expandida por §2.1 — qualquer call sem p_source teria violado o CHECK.
-- Todos os callsites em v2.9 passam p_source explicitamente.
...
BEGIN
  ...
  INSERT INTO job_canonical_skills (
    label, status, source, employer_id, vacancy_count, distinct_sources_count
  ) VALUES (
    trim(p_label), 'pending', p_source, p_employer_id, 0, 0
  )
  RETURNING id INTO v_id;
...
### §2.18 — Índices parciais simétricos JCS+JCR

**Arquivo:** `18_indices_parciais_lookup_pending.sql`
**Atende:** S-SYM-4.

```sql
BEGIN;

CREATE INDEX IF NOT EXISTS idx_jcs_lookup_label_lower
  ON job_canonical_skills (lower(label), status)
  WHERE status IN ('active', 'pending') AND merged_into IS NULL;

CREATE INDEX IF NOT EXISTS idx_jcr_lookup_canonical_label_lower
  ON job_canonical_roles (lower(canonical_label), status)
  WHERE status IN ('active', 'pending') AND merged_into IS NULL;
...
### §2.20 — `set_pipeline_config_value` atualizada

**Arquivo:** `20_set_pipeline_config_value.sql`
**Bloqueadores resolvidos:** B4 (grava `pipeline_config_history` com nomes reais `changed_by`/`reason`, parâmetro real `p_value`).

**Ground truth confirmado:** o trigger `trg_pipeline_config_audit` (AFTER UPDATE ON pipeline_config → `fn_pipeline_config_audit()`) existe no banco. Sem o DROP abaixo, cada chamada a `set_pipeline_config_value` produz **2 entradas** em `pipeline_config_history`: uma do trigger (com `changed_by = SYSTEM_USER_ID` errado) e uma do INSERT explícito desta função (com `changed_by = p_changed_by` correto). O trigger deve ser dropado antes de recriar a função — a RPC passa a ser o **único** caminho de escrita no histórico, consistente com o RLS FORCE de §2.20a.

```sql
BEGIN;

-- Remover trigger de auditoria legado: a RPC set_pipeline_config_value é o único
-- caminho de gravação em pipeline_config_history (ver §2.20a RLS FORCE).
-- Sem este DROP, cada UPDATE via RPC gera 2 entradas no histórico.
...
### §2.20a — RLS FORCE em `pipeline_config_history` (auditoria imutável)

**Arquivo:** `20a_pipeline_config_history_rls_force.sql`
**Objetivo:** garantir que histórico de alterações em `pipeline_config` é write-once-by-RPC. Mesmo o owner (service_role) não consegue UPDATE/DELETE direto na tabela — apenas o RPC `set_pipeline_config_value` (que roda como SECURITY DEFINER) faz INSERT. Atende ao requisito do mockup: "auditoria imutável via RLS FORCE".
...
-- Política de SELECT: admin pode ler tudo
...
-- Política de INSERT: bloqueada para todos (apenas SECURITY DEFINER bypassa RLS)
-- Nenhuma policy de INSERT/UPDATE/DELETE — RLS bloqueia por default

-- Garantir que o RPC set_pipeline_config_value é o único caminho de gravação
-- (já é SECURITY DEFINER em §2.20)

COMMIT;
```

**Validação:** `INSERT INTO pipeline_config_history (...) VALUES (...)` como usuário authenticated retorna erro de RLS. Apenas chamadas via `SELECT set_pipeline_config_value(...)` (que roda em definer context) inserem.
...
-- Arquitetura dois-triggers por entidade (confirmar no DB antes de aplicar):
-- ┌─ BEFORE UPDATE OF confidence_median ── fn_flag_needs_opus_review_jcr()   ← recriado aqui
-- └─ AFTER  UPDATE OF needs_opus_review  ── fn_flag_needs_opus_review_jcr_emit_event() ← NÃO TOCAR
-- Idem para JCS. Os triggers _emit (trg_flag_needs_opus_review_jcr_emit / _jcs_emit) são
-- AFTER triggers que emitem o evento 'role_flagged_low_confidence_residual' /
-- 'skill_flagged_low_confidence_residual' quando needs_opus_review muda FALSE→TRUE.
-- Eles continuam corretos após esta migration — NÃO DROPAR.
-- Esta migration apenas expande os BEFORE triggers para também escutar OF status.

-- Recriar BEFORE triggers com OF confidence_median, status (antes era OF confidence_median apenas)
...
### §2.25 — UNIQUE em `resume_roles(resume_id, role)`

**Arquivo:** `25_resume_roles_unique.sql`
**Atende:** S-SYM-6 (UPSERT idempotente).

```sql
BEGIN;

-- Deduplicação prévia (base limpa, mas defensivo)
DELETE FROM resume_roles a
USING resume_roles b
WHERE a.id > b.id
  AND a.resume_id = b.resume_id
  AND lower(trim(a.role)) = lower(trim(b.role));

CREATE UNIQUE INDEX IF NOT EXISTS uq_resume_roles_resume_id_role
  ON resume_roles (resume_id, lower(trim(role)));
...
COMMENT ON TABLE taxonomy_blacklist IS
  'Lista de labels banidos da criação automática de canonicals (roles/skills). Sprint atual: apenas inserções via merge-canonicals split-button "Rejeitar e banir". Sprint futura: CRUD completo via tela dedicada.';
...
**Integração futura com criação de canonicals:** a partir da próxima sprint, funções como `reconcile_canonical_role` e `reconcile_canonical_skill` (§2.13, §2.14) devem checar `taxonomy_blacklist` antes de criar canonical pending. Caso o label normalizado esteja banido, abortar com `RAISE EXCEPTION` específica registrando rejeição. **Fora do escopo desta sprint** — apenas tabela criada; check ainda não aplicado.
...
### §2.36 — RPC `reject_and_blacklist_canonical_pair` (split-button "Rejeitar e banir")

**Arquivo:** `36_reject_and_blacklist_canonical_pair.sql`
...
**Atende:** simetria com §3.3 — extração de skills do currículo, hard gate em batch, resolução de canonical_skill_id, persistência em `resume_skills` (tabela criada em §2.8b). Substitui o destino anterior (`analysis_skill_matches`) cujo schema é incompatível com store de skills brutas pós-extração.
...
- Parâmetro renomeado: `analysisId` → `resumeId` (vincula skill ao currículo, não a uma análise específica).
- Parâmetro `canonicalRoleId?` removido — não faz sentido aqui (role pertence ao currículo via `resume_roles`, não às skills).
- Usa `filterByHardGate` em vez de leitura inline.
- Adiciona `hardGateMin` e `canonicalsResolved` ao retorno.
- Adiciona dedup por nome normalizado pré-INSERT (espelha §3.3 — proteção contra unique violation no índice funcional de §2.8b).
- Resolve `canonical_skill_id` via RPC `reconcile_canonical_skill` antes do INSERT.
- UPSERT com `onConflict: 'resume_id,skill'`.
...
export async function extractAndPersistResumeSkills(
  ...
  // Filtra ANTES do loop de canonização (S-SYM-9)
  const passed = gated.filter((s) => s.passedHardGate);

  // Dedup por nome normalizado contra o índice funcional de §2.8b
  // UNIQUE INDEX em (resume_id, lower(trim(skill))) → duplicatas pós-normalização
  // causariam unique violation no INSERT de job_canonical_skills (§2.14 sem ON CONFLICT).
  const dedupedByNormalized = new Map<string, typeof passed[number]>();
  for (const s of passed) {
    const normalized = s.name.trim().toLowerCase();
    if (!normalized) continue; // descarta strings vazias pós-trim
    const existing = dedupedByNormalized.get(normalized);
    if (!existing || s.confidence > existing.confidence) {
      dedupedByNormalized.set(normalized, s);
    }
  }
  const passedDedup = Array.from(dedupedByNormalized.values());

  let canonicalsResolved = 0;

  if (passedDedup.length > 0) {
    const resolved = await Promise.all(
      passedDedup.map(async (s) => {
        const { data, error } = await supabase.rpc('reconcile_canonical_skill', {
          p_label: s.name,
          p_employer_id: null,
          p_source: 'resume_extraction',
        });
        if (error) throw new Error(`reconcile_canonical_skill failed for "${s.name}": ${error.message}`);
    ...
    const { error: upsertError } = await supabase
      .from('resume_skills')
      .upsert(rows, { onConflict: 'resume_id,skill', ignoreDuplicates: false });

    if (upsertError) throw new Error(`resume_skills upsert failed: ${upsertError.message}`);

    canonicalsResolved = resolved.length;
  }

  return {
    hardGateMin: threshold,
    totalExtracted: skills.length,
    passedHardGate: passed.length,
    rejectedHardGate: skills.length - passed.length,
    canonicalsResolved,
  };
}
```

**Caller (`app/api/...` ou fluxo de upload):** atualizar chamadas existentes — passar `resumeId` em vez de `analysisId`; remover argumento `canonicalRoleId` (não usado mais). Ajuste cirúrgico de 1-2 linhas por call site.
...
export async function extractAndPersistResumeRoles(
  ...
  // Filtra ANTES do loop de canonização (S-SYM-9)
  const passed = gated.filter((r) => r.passedHardGate);

  // ─── M4 + M5: pré-normalização + dedup contra o índice funcional ───
  // §2.25 cria UNIQUE INDEX em (resume_id, lower(trim(role))).
  // Para o upsert via Supabase JS funcionar com esse índice, garantimos que
  // duplicatas pós-normalização não sejam enviadas simultaneamente — Promise.all
  // com 2 entradas resolvendo o mesmo canonical causaria unique violation no INSERT
  // de job_canonical_roles (§2.13 não tem ON CONFLICT). Pré-normaliza + dedup por
  // label normalizado, mantendo o label original com maior confidence em caso de empate.
  const dedupedByNormalized = new Map<string, typeof passed[number]>();
  ...
  if (passedDedup.length > 0) {
    const resolved = await Promise.all(
      passedDedup.map(async (r) => {
        const { data, error } = await supabase.rpc('reconcile_canonical_role', {
          p_canonical_label: r.role,
          p_employer_id: null,
          p_source: 'resume_extraction',
        });
        if (error) throw new Error(`reconcile_canonical_role failed for "${r.role}": ${error.message}`);
    ...
    const rows = resolved.map((r) => ({
      ...
      source: 'resume_extraction', // requerido pelo CHECK constraint job_canonical_roles_source_check (§2.2) e pelo smoke test S2
    }));

    const { error: upsertError } = await supabase
      .from('resume_roles')
      .upsert(rows, { onConflict: 'resume_id,role', ignoreDuplicates: false });

    if (upsertError) throw new Error(`resume_roles upsert failed: ${upsertError.message}`);
  ...
  return {
    ...
    passedHardGate: passed.length,
    rejectedHardGate: roles.length - passed.length,
    canonicalsResolved,
  };
}
```

**Nota crítica sobre `onConflict` + índice funcional (M4):** o índice de §2.25 (`uq_resume_roles_resume_id_role ON (resume_id, lower(trim(role)))`) é funcional. O comportamento de `onConflict: 'resume_id,role'` via Supabase JS / PostgREST com índices funcionais é não-determinístico — pode tratar como conflito ou ignorar silenciosamente. A pré-normalização + dedup acima garante consistência mesmo nesse cenário: ao gravar cada `r.role` como veio (preservando capitalização original), o índice funcional resolve a unicidade no banco. Em produção, monitorar erros HTTP 409 nesse endpoint; se aparecerem, migrar para RPC dedicada `upsert_resume_role(p_resume_id uuid, p_role text)` com `INSERT ... ON CONFLICT (resume_id, lower(trim(role))) DO UPDATE`.
...
**Assinatura correta:** `(supabase, resumeId, skills)` — A3 ground truth + §3.2 consolidado.

Localização da edição: chamada existente de `extractAndPersistResumeSkills`. Atualizar a chamada para usar **3 argumentos** `(supabase, resumeId, skills)` — o parâmetro `canonicalRoleId?` foi explicitamente removido em §3.2 (o role pertence ao currículo via `resume_roles`, não às skills). Qualquer argumento 4º deve ser removido do call site; passá-lo causaria erro de compilação TypeScript após a renomeação e mudança de assinatura de §3.2.
...
- `PATCH /api/admin/pipeline-config/[key]` — body: `{ value, reason, confirmed }`. Chama `set_pipeline_config_value` com `p_confirmed = body.confirmed`. Retorna erro 400 se `criticality_level = 'high'` e `confirmed != true`.
...
**Guard contra rollback de seed inicial:** o handler DEVE validar `if (!target.previous_value) return 400` antes de chamar a RPC. A entrada de seed inicial tem `previous_value IS NULL` (não havia valor anterior). Reverter para `NULL` quebraria todas as leituras que fazem `value::int` ou `value::numeric`. A UI bloqueia o botão de rollback no seed inicial (§5.1.7), mas o handler precisa do guard correspondente — chamada direta via curl contornaria a proteção do front. Erro 400 com mensagem: `"Não é possível reverter para o estado anterior ao seed inicial (valor era NULL)."`
...
- `POST /api/admin/backfill/canonical-domains` — body: `{ dryRun }`. Dispara `backfillCanonicalDomains` da §3.9.
- `GET /api/admin/backfill/canonical-domains/status` — acompanhamento de progresso. Retorna `{ total_roles_active, roles_with_domain, pct_covered }`. Implementação:
...
Para alimentar §5.1.4 e §5.1.6, `pipeline_config` precisa ter coluna `description text`. Migration §2.19a (acréscimo):

```sql
ALTER TABLE pipeline_config ADD COLUMN IF NOT EXISTS description text;

UPDATE pipeline_config SET description = 'Teto da Zona Opus para habilidades' WHERE key = 'skill.promotion.auto_min_confidence';
UPDATE pipeline_config SET description = 'Piso de aceitação de habilidade nova' WHERE key = 'skill.hard_gate.min_confidence';
UPDATE pipeline_config SET description = 'Mínimo de vagas para promoção pending→active' WHERE key = 'role.promotion.min_vacancies';
-- ... seed para todas as 24 chaves
```

A spec deixa o seed completo das 24 descriptions como item da implementação (depende de revisão do PO sobre o texto final de cada label).
...
-- E5: índices simétricos criados
SELECT indexname FROM pg_indexes
WHERE indexname IN (
  'idx_jcs_lookup_label_lower',
  'idx_jcr_lookup_canonical_label_lower'
);
-- Esperado: 2 linhas
...
-- Tipo de resume_roles.confidence migrado para numeric (§2.7a)
SELECT column_name, data_type, numeric_precision, numeric_scale
FROM information_schema.columns
WHERE table_name = 'resume_roles' AND column_name = 'confidence';
-- Esperado: data_type='numeric', numeric_precision=5, numeric_scale=4
-- (Antes desta sprint era smallint — perda de precisão ao inserir float 0-1 do LLM)

-- trigger de auditoria legado removido (§2.20)
SELECT tgname FROM pg_trigger WHERE tgrelid = 'pipeline_config'::regclass;
-- Esperado: 0 linhas (trigger trg_pipeline_config_audit foi dropado; gravação exclusiva via RPC).
...
-- Constraint NOT NULL + CHECK em resume_skills
SELECT conname, pg_get_constraintdef(oid) AS def
FROM pg_constraint
WHERE conname IN (
  'resume_skills_passed_hard_gate_consistency'
);
-- Esperado: 1 linha; def contém 'confidence >= 0.70'.

-- Índices em resume_skills (UNIQUE funcional + 2 parciais)
SELECT indexname FROM pg_indexes
WHERE tablename = 'resume_skills'
  AND indexname IN (
    'uniq_resume_skills_resume_skill_normalized',
    'idx_resume_skills_canonical_skill_id',
    'idx_resume_skills_passed_hard_gate'
  );
-- Esperado: 3 linhas.

-- CHECKs atualizados (§2.1 e §2.2 preservam os nomes originais dos constraints)
SELECT conname, pg_get_constraintdef(oid) AS def
FROM pg_constraint
...
-- Esperado: 2 linhas — ambos os constraints agora incluem 'resume_extraction'.
-- job_canonical_skills_source_check: 5 valores (llm_extractor, cbo_mte_2002_seed, manual_admin, opus_arbitration, resume_extraction)
-- job_canonical_roles_source_check: 7 valores (seed, llm_extractor, cbo_mte_2002_seed, job_posting, resume_extraction, manual_admin, opus_arbitration)
```
...
--   roles_with_source_set = total_roles (D-PS-15 corrigido).
...
| A1.3 | Endpoint `POST /api/admin/backfill/canonical-domains` com `dryRun=true` retorna contagem; sem dryRun popula `canonical_role_domain_links` |
...
**Parcial:**
- Cada migration tem snapshot `pg_get_functiondef` prévio salvo em `docs/migrations/sprint-cleanup-v2/snapshots/NN_pre.sql`. Rollback de uma migration individual = aplicar o snapshot correspondente.
...
```
SUB-PR-1: §2.1, §2.2, §2.3, §2.7a, §2.7, §2.8, §2.8b, §2.25, §2.31, §2.36 (schema additions sem dependências + RPC blacklist contextual + resume_skills paralela)
   Nota ordem obrigatória dentro do SUB-PR-1: §2.7a (ALTER COLUMN confidence para numeric) DEVE preceder §2.8 (que adiciona CHECK confidence >= 0.70 com semântica float 0-1).
   ↓
SUB-PR-2: §2.4, §2.5, §2.6, §2.15, §2.18 (lookups + índices)
   ↓
...
SUB-PR-6: §2.23, §2.24 (catchups + Opção C — depende de SUB-PR-4)
   ↓
SUB-PR-7: §2.9 (trigger latest_posted_at JCR)
   ↓
SUB-PR-8: §2.16, §2.17, §2.19, §2.19a, §2.20, §2.20a (views + registry + pipeline_config completo)
   │       Inclui:
   ...
   │       - §2.20a RLS FORCE em pipeline_config_history
   ↓
...
- **D-PS-22:** Snapshots `pg_get_functiondef` pré-migration obrigatórios em `docs/migrations/sprint-cleanup-v2/snapshots/`.
...
- **LK-PS-17:** §7.1 + §7.1.1 (evidence post-deploy + smoke tests granulares) rodam após cada SUB-PR conforme cabíveis. Saídas armazenadas em `docs/migrations/sprint-cleanup-v2/evidence-post-deploy.sql.out` para auditoria.
...
**Fim da SPEC-sprint-cleanup-v2.6.**
