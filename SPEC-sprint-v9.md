# Sprint v9.0 — Fluxo de Retorno + Governança de Taxonomia + Limpeza Estrutural
...
describe('Item 16 — DROP de colunas redundantes', () => {
    ...
    it('shared_results.final_score foi DROPADA (era snapshot dormente, premissa falsa da v1)', async () => {
        // teste invertido — Item 16 dropa final_score (decisão #14 do changelog v1→v2)
 ...
 * 
 * Sobre este wrapper:
 * mas esqueceu este wrapper legacy que continuava fazendo `split('-')` em tokens individuais
 * (linha equivalente da v8). Resultado: se algum dos 4 callsites legacy chamasse
 * `getFullTaxonomyCache().familySynonyms`, falsos positivos catastróficos voltavam — 
 * vaga "Engenheiro de Vendas" → token `engenheiro` → set gigantesco de canônicos dispares.
 * 
 * v9: chave do Map é a frase contígua (ex.: 'engenheiro-de-software' → 'engenheiro de software'),
 * idêntica à do helper novo. Match no callsite legacy DEVE ser via `titleLower.includes(familyPhrase)`,
 * não busca direta por token. Se algum callsite legacy ainda assumir busca por token, 
 * precisa ser migrado para findFamilyMatches() diretamente.
 * 
 * Valores do Map continuam sendo `string[]` de **labels** dos canônicos (não IDs).
 */
