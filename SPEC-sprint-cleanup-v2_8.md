...
CREATE OR REPLACE FUNCTION reconcile_canonical_role(
  p_canonical_label text,
  p_employer_id uuid,
  p_source text DEFAULT 'job_posting'
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION reconcile_canonical_skill(
  p_label text,
  p_employer_id uuid,
  p_source text DEFAULT 'job_posting_skill'
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
export async function extractAndPersistResumeRoles(
  ...
  if (passedDedup.length > 0) {
    ...
    const rows = resolved.map((r) => ({
      ...
      source: 'resume_extraction', // requerido pelo CHECK constraint jcr_source_check (§2.2) e pelo smoke test S2
    }));
...
```
SUB-PR-1: §2.1, §2.2, §2.3, §2.7, §2.8, §2.8b, §2.25, §2.31, §2.36 (schema additions sem dependências + RPC blacklist contextual + resume_skills paralela)
   ↓
