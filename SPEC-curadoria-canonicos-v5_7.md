...
1. Patch no `domain_synonyms.json` v2.0 → v3.0 com 65 entradas novas. Baseline real de 113 entradas existentes. Total final: 178.
...
Esta v5.7 **não** resolve os seguintes problemas, deliberadamente:
...
4. **Convergência semântica em títulos ambíguos fora do patch §6.1.** As 65 entradas novas cobrem os casos de alto volume identificados empiricamente. Títulos ambíguos não cobertos (ex: "Solution Consultant" — delivery vs pre-sales) continuam dependendo do LLM guiado pelas Regras A-E.
...
│ • SEM fallback por embedding nesta v5.7                      │
└─────────────────────────────────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────────────┐
...
-- 2. TRUNCATE + INSERT INTO ... SELECT * FROM _backup_v5_7
...
CREATE TABLE job_postings_backup_v5_7 AS
    SELECT * FROM job_postings;
COMMENT ON TABLE job_postings_backup_v5_7 IS
    'Backup tirado em [timestamp no momento de aplicação] como safety net para rollback da v5.6';

CREATE TABLE job_canonical_roles_backup_v5_7 AS
    SELECT * FROM job_canonical_roles;
COMMENT ON TABLE job_canonical_roles_backup_v5_7 IS
    'Backup tirado em [timestamp no momento de aplicação] como safety net para rollback da v5.6';
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'CREATE TABLE skill_merge_decisions_backup_v5_7 AS
                 SELECT * FROM skill_merge_decisions';
        EXECUTE 'COMMENT ON TABLE skill_merge_decisions_backup_v5_7 IS ''Backup para rollback v5.6''';
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'CREATE TABLE role_merge_decisions_backup_v5_7 AS
                 SELECT * FROM role_merge_decisions';
        EXECUTE 'COMMENT ON TABLE role_merge_decisions_backup_v5_7 IS ''Backup para rollback v5.6''';
...
BEGIN
    ...
    SELECT COUNT(*) INTO bkp_jp FROM job_postings_backup_v5_7;
    SELECT COUNT(*) INTO orig_jcr FROM job_canonical_roles;
    SELECT COUNT(*) INTO bkp_jcr FROM job_canonical_roles_backup_v5_7;
    ...
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'skill_merge_decisions_backup_v5_7'
    ) INTO smd_exists;

    IF smd_exists THEN
        SELECT COUNT(*) INTO orig_smd FROM skill_merge_decisions;
        SELECT COUNT(*) INTO bkp_smd FROM skill_merge_decisions_backup_v5_7;
        IF orig_smd != bkp_smd THEN RAISE EXCEPTION 'Backup skill_merge_decisions incompleto: orig=%, bkp=%', orig_smd, bkp_smd; END IF;
    END IF;

    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'role_merge_decisions_backup_v5_7'
    ) INTO rmd_exists;

    IF rmd_exists THEN
        SELECT COUNT(*) INTO orig_rmd FROM role_merge_decisions;
        SELECT COUNT(*) INTO bkp_rmd FROM role_merge_decisions_backup_v5_7;
...
-- v5.7 (Claude Code B5): runbook completo de restore caso houve UPDATE
-- parcial em linhas existentes ou caso o backup precise ser materializado.
-- O caso típico de rollback da v5.7 é DROP COLUMN sem precisar restaurar
-- dados (porque tudo que a §5.1 faz é ADD COLUMN), mas em caso de
-- corrupção parcial:

-- Passo 1: drop dos triggers/índices novos antes de restaurar dados
...
DROP TRIGGER IF EXISTS trg_sync_canonical_role_label ON job_canonical_roles;
DROP INDEX IF EXISTS idx_job_postings_description_hash_precheck;
DROP INDEX IF EXISTS idx_job_postings_human_validated;
DROP INDEX IF EXISTS idx_job_postings_curation_source;

-- Passo 2: TRUNCATE + INSERT explícito com lista de colunas pré-v5.4
-- A lista abaixo é a do schema PRÉ-migration (sem as 10 colunas novas).
-- Se a tabela tem outras colunas custom adicionadas em sprints anteriores,
-- ajustar conforme schema atual antes do rollback.
TRUNCATE job_postings;

INSERT INTO job_postings (
    id, source_id, title, requirements, company, location,
    seniority, work_model, posted_at, scraped_at,
    canonical_role_id, curated_at, curation_status,
    fingerprint, source, source_url, raw_payload,
    description_curated, seniority_inferred,
    skills, origin_mode, created_at, updated_at
)
SELECT
    id, source_id, title, requirements, company, location,
    seniority, work_model, posted_at, scraped_at,
    ...
    description_curated, seniority_inferred,
    skills, origin_mode, created_at, updated_at
FROM job_postings_backup_v5_7;

-- Passo 3: recriar índices, triggers e RLS policies que já existiam pré-v5.4
-- (consultar docs/schema.sql para a lista canônica). NOTA: este passo é
-- responsabilidade do executor do rollback validar contra o schema atual,
-- já que o backup CREATE TABLE AS SELECT * não copia esses artefatos.
...
```sql
DROP TABLE IF EXISTS job_postings_backup_v5_7;
DROP TABLE IF EXISTS job_canonical_roles_backup_v5_7;
DROP TABLE IF EXISTS skill_merge_decisions_backup_v5_7;
DROP TABLE IF EXISTS role_merge_decisions_backup_v5_7;
```
...
### 6.1 `domain_synonyms.json` — bump 2.0 → 3.0 com 65 entradas novas

Entradas a adicionar ao objeto `data`, preservando as **113 entradas existentes** intactas. Bump do campo `version` de `"2.0"` para `"3.0"`. Total final: 178 entradas.

**Contagem por categoria: 23 + 14 + 7 + 14 + 7 = 65 entradas novas.**
...
**Categoria E — Sinônimos decorrentes das operações de taxonomia cravadas no Anexo B (v5.7 — 7 entradas novas)**
...
**v5.7 (Claude Code) — Correção crítica de build:** a v5.5 tinha 10 entradas em Category E mas 3 delas formavam pares acento/sem-acento que `normalizeTitle` colapsa para a mesma string (NFD + strip diacríticos + hífen→espaço). O teste de higiene `tests/domain-synonyms-hygiene.test.ts` (§7.4.0.1, N1) detecta duplicatas pós-normalização e falha o build. Como o `prebuild` roda esse teste, o build não passa em produção.

Pares colapsantes removidos:
- `"representante de pre-venda"` (duplicata de `"representante de pré-venda"` após normalização)
- `"pre-vendas"` (duplicata de `"pré-vendas"`)
- `"analista de pre-vendas"` (duplicata de `"analista de pré-vendas"`)

**Cobertura preservada:** títulos brutos sem acento (ex: "Representante de Pre-Venda" como digitado em alguns ATS) ainda matcham porque `normalizeTitle` é aplicado tanto na key do JSON quanto no título da vaga antes da regex compilada. As 3 keys com acento cobrem ambas as formas via normalização.

**Contagem v5.7:** categorias A (23) + B (14) + C (7) + D (14) + E (7) = **65 entradas novas**. Total final: 113 existentes + 65 novas = **178 entradas**.
...
**Implicação operacional:** ao editar `domain_synonyms.json`, o autor DEVE escrever as keys como a saída dessa função. Para as 65 entradas novas do patch §6.1, isso já foi validado: `"head of fp&a"` vira `"head of fp and a"` na normalização, então a key precisa estar escrita como `"head of fp&a"` no JSON (a transformação `&` → ` and ` ocorre no runtime). Mais simples: keys no JSON podem manter `&` original porque a normalização converte runtime. **O que não pode**: key com acento, com caixa mista, ou com barras/hífens — porque essas formas não sobrevivem à normalização.
...
export interface RunCounters {
    ...
    // v5.7 (Claude Code race §7.11): vagas onde persistCuratedJob detectou
    // race condition (curation_status já não era 'pending' ao tentar UPDATE).
    // Visibilidade no resumo do batch para detectar concorrência admin/pipeline.
    persistCuratedRaceConflict: number;
}
...
export function createRunCounters(): RunCounters {
    return {
        ...
        persistCuratedRaceConflict: 0,  // v5.7 (Claude Code race §7.11)
    };
}
```
...
| `persistCuratedRaceConflict` | `persist-curation.ts` §7.11 | dentro de `if (!data || data.length === 0)` após UPDATE com guard `.eq('curation_status', 'pending')` | v5.7: vaga já não estava em `pending` no momento do UPDATE — admin curou em paralelo ou outro caminho do pipeline tocou a vaga |
...
for (const job of jobsInRollout) {
    // v5.7 (Claude Code B4): preferir original_description (campo populado
    // pelo pipeline de ingestão pós-v5.4) com fallback para requirements.description
    // durante transição. Após sprint futura de remoção de requirements.description,
    // o fallback cai. Sem o fallback hoje, vagas legadas pré-backfill da §5.1
    // ficariam com hash de string vazia → cluster gigante de duplicatas inválido.
    const requirements = job.requirements as { description?: string } | null;
    const description = (job as any).original_description ?? requirements?.description ?? '';
...
import { SupabaseClient } from '@supabase/supabase-js';
import { SYSTEM_USER_ID } from '@/lib/pipeline/constants';
// v5.7 (Claude Code N5): SYSTEM_USER_ID importado explicitamente, em vez de
// hardcoded em strings ao longo do arquivo (consistência com §7.7.2).
...
export async function persistPrecheckResult(
    ...
    //
    // v5.7 (Claude Code N3): guard inclui anchorJobId. Se caller passar
    // anchorUpdatedAt mas omitir anchorJobId, query roda com
    // .eq('id', undefined) e retorna vazio com erro críptico.
    if (input.anchorUpdatedAt && input.anchorJobId) {
        ...
        if (anchorNow.updated_at !== input.anchorUpdatedAt) {
            ...
            await supabase.from('events').insert({
                ...
                actor: 'pipeline',
                actor_id: SYSTEM_USER_ID,  // v5.7 (Claude Code N5): import em vez de hardcoded
...
//
// v5.7 (Claude Code N4): imports explícitos no topo do snippet.
// O snippet original omitia os imports porque a função existente já os tinha,
// mas implementador que copia o snippet isoladamente compila com erro.
import { SupabaseClient } from '@supabase/supabase-js';
import { SYSTEM_USER_ID } from '@/lib/pipeline/constants';
import { PROMPT_STRUCTURE_VERSION, PROMPT_CONTENT_VERSION } from '@/lib/pipeline/prompt-version.generated';
...
export async function persistCuratedJob(
    ...
    // v5.7 (Claude Code race §7.11): detectar race condition igual ao
    // persistPrecheckResult (§7.10). Sem isso, se admin curou a vaga
    // manualmente entre a chamada do LLM e este UPDATE, retornaríamos
    // sucesso silencioso (UPDATE afeta 0 linhas mas não retorna error).
    if (!data || data.length === 0) {
        await supabase.from('events').insert({
            event_name: 'persist_curated_update_conflict',
            resource_type: 'job_posting',
            resource_id: normalized.jobId,
            actor: 'pipeline',
            actor_id: SYSTEM_USER_ID,
            reason: 'Job não estava em curation_status=pending ao tentar persistir resultado do LLM — admin curou manualmente em paralelo?',
            metadata: {
                canonical_role_label: normalized.canonicalRole,
                curation_source: options.curation_source ?? 'llm_direct'
            }
        });
        // v5.7: contador agregado em RunCounters para visibilidade no resumo do batch.
        // Adicionado `persistCuratedRaceConflict` ao tipo RunCounters (§7.11.0).
        if (counters) {
            counters.persistCuratedRaceConflict++;
        }
        // Não falhar o batch: a vaga está curada (mesmo que por outro caminho).
    }
...
**Arquivo:** `docs/migrations/20260424_03_canonical_role_label_sync_trigger.sql`

**v5.7 (Claude Code B1+B2):** path corrigido de `20260423_04_*` (que colidia com migration existente `20260423_04_sprint0_cleanup_transaction.sql`) para `20260424_03_*` (data atual, sequencial após `20260424_02_*` do trigger N4). Nome do trigger renomeado de `trigger_sync_canonical_role_label_after_update` para `trg_sync_canonical_role_label` para alinhar com convenção `trg_*` do projeto e com a validação do Passo 12.14.
...
-- v5.7 (Claude Code B1): nome alinhado à convenção trg_* do projeto e à
-- validação do Passo 12.14 (que espera 'trg_sync_canonical_role_label').
DROP TRIGGER IF EXISTS trigger_sync_canonical_role_label_after_update
ON job_canonical_roles;
DROP TRIGGER IF EXISTS trg_sync_canonical_role_label ON job_canonical_roles;

CREATE TRIGGER trg_sync_canonical_role_label
AFTER UPDATE ON job_canonical_roles
FOR EACH ROW
EXECUTE FUNCTION sync_canonical_role_label_on_update();

COMMENT ON TRIGGER trg_sync_canonical_role_label
...
2. Aplicar patch em `domain_synonyms.json` (v2.0 → v3.0 com 65 entradas; total final: 178)
...
- [ ] Contagem: `jq '.data | length'` retorna 178 (113 + 65).
...
- [ ] **v5.2 — Teste `tests/domain-synonyms-hygiene.test.ts` (N1):** todas as 178 keys sobrevivem a `normalizeTitle` sem perder mais de 50% dos caracteres; não há duplicatas após normalização; todos os labels são strings não-vazias.
...
- [ ] **v5.7 — Trigger `trg_sync_canonical_role_label` (N3):** criado; UPDATE em `job_canonical_roles.label` propaga para `job_postings.canonical_role_label`; evento `canonical_role_label_synced` é gravado.
...
- [ ] §2.1 — Validação de estado da migration §5.1: ANTES do Passo 12.2, query `information_schema.columns` para `original_title` e `original_description` em `job_postings` retorna **0 linhas** (colunas ainda não existem). APÓS Passo 12.2, mesma query retorna **2 linhas** (ambas criadas pela §5.1 + backfilled). v5.7 (Claude Code rodada atual) — alinhado com §2.1 que esclarece que essas colunas **são criadas pela própria migration**, não pré-requisito de produção
...
- [ ] Aplicar patch em `domain_synonyms.json` com bump 2.0 → 3.0 e 65 entradas novas
- [ ] Validar contagem final: 113 + 65 = 178
...
|---|---|---|---|
| 12.1 | **Backup** | Executar `CREATE TABLE ... _backup_v5_7 AS SELECT *` para as 4 tabelas (condicional nas 2 de merge_decisions) + DO block de validação de contagens | §5.0 |
...
| 12.9 | **Trigger N3 — sincronia `canonical_role_label`** | Aplicar `docs/migrations/20260424_03_canonical_role_label_sync_trigger.sql` | §7.14 |
...
| 12.17 | **Smoke test do Trigger N3** | Selecionar canônico real do catálogo com >100 vagas atreladas; medir tempo do `UPDATE job_canonical_roles SET label = '<novo>' WHERE id = ...`. Validar que tempo é razoável (sub-segundo no típico) e que `canonical_role_label` propaga em todas as vagas. v5.7 (Claude Code rodada atual) | §11.3 |
...
- [ ] **12.4 (v5.5 — DeepSeek #1, corrigido v5.7 — Claude Code rodada atual)** — Aplicar migração §5.1.2: função `enforce_original_immutability()` + trigger `BEFORE UPDATE` em `job_postings`. Testar silenciamento (não EXCEPTION — a v5.6 trocou EXCEPTION por silenciamento defensivo via Gemini #1): rodar `UPDATE job_postings SET original_title = 'teste-de-trigger' WHERE id = <qualquer-vaga-com-original_title-populado>` deve retornar sucesso (1 linha afetada) MAS o valor real de `original_title` permanece inalterado. Confirmar com `SELECT original_title FROM job_postings WHERE id = <mesma-vaga>` — valor original preservado. `RAISE NOTICE` deve aparecer nos logs do Postgres com mensagem "Tentativa de alterar original_title silenciada na vaga..."
...
- [ ] **12.17 (v5.7 — Claude Code rodada atual)** — **Smoke test do Trigger N3 (sincronia de `canonical_role_label`):** validar que rename de canônico com volume real propaga em tempo aceitável e sem travar o pipeline. Como executar:

```sql
-- 1. Selecionar canônico do catálogo com >100 vagas (não precisa ser o de
-- maior volume — basta um representativo do comportamento real)
SELECT id, label, vacancy_count
FROM job_canonical_roles
WHERE status IN ('active', 'pending')
  AND vacancy_count >= 100
ORDER BY vacancy_count
LIMIT 1;

-- 2. Capturar timestamp ANTES do rename
SELECT NOW() AS antes_rename;

-- 3. Executar rename (substituir <id_canonico> e <novo_label> pelo retornado acima)
-- Sufixar com '-smoke-test' para reverter facilmente depois:
UPDATE job_canonical_roles
SET label = label || '-smoke-test'
WHERE id = '<id_canonico>';

-- 4. Capturar timestamp DEPOIS do rename
SELECT NOW() AS depois_rename;

-- 5. Validar propagação: contagem de vagas com label novo deve bater com vacancy_count
SELECT COUNT(*) AS vagas_propagadas
FROM job_postings
WHERE canonical_role_id = '<id_canonico>'
  AND canonical_role_label LIKE '%-smoke-test';

-- 6. Validar evento forense em events
SELECT event_name, reason, metadata, created_at
FROM events
WHERE event_name = 'canonical_role_label_synced'
  AND resource_id = '<id_canonico>'
ORDER BY created_at DESC
LIMIT 1;

-- 7. Reverter o rename (limpeza)
UPDATE job_canonical_roles
SET label = REPLACE(label, '-smoke-test', '')
WHERE id = '<id_canonico>';
```

**Critérios de aceite:**
- Tempo entre `antes_rename` e `depois_rename` < 5 segundos para canônico com até 1.000 vagas (referência: a maioria dos canônicos do catálogo está nessa faixa)
- `vagas_propagadas` = `vacancy_count` capturado no passo 1 (propagação completa)
- 1 evento `canonical_role_label_synced` gravado com `old_label` e `new_label` corretos
- Após reverter, `canonical_role_label` em todas as vagas volta ao valor original (segundo `canonical_role_label_synced` evento gravado)

**Se o tempo exceder 5 segundos** ou propagação ficar incompleta: investigar lock contention concorrente com CRON de ingestão. Mitigação imediata: pausar CRON de ingestão durante operações de rename via tela admin (já documentado no §11.1 como aviso operacional). Se mesmo com CRON pausado o tempo for problemático, considerar otimização de índice em `job_postings.canonical_role_id`.
...
## Anexo B — Decisões §2.4 cravadas (v5.7 final)
...
Incluir no patch do PR1 (se não estiverem ainda). v5.7 (Claude Code): listas de sinônimos abaixo só incluem keys que sobrevivem ao teste de higiene `tests/domain-synonyms-hygiene.test.ts`. Variantes acento/sem-acento que colapsam após `normalizeTitle` são intencionalmente omitidas porque a key acentuada cobre as duas formas via normalização.

**Para "SDR":**
- `"sdr"`, `"business development representative"`, `"sales development representative"`, `"representante de pré-venda"`, `"pré-vendas"`, `"analista de pré-vendas"`

**Para "BDR":**
- `"bdr"`, `"business development associate"`, `"bda"`, `"analista de desenvolvimento de negócios"`

**Para "Analista de Operações de Vendas":**
- `"sales operations"`, `"sales ops"`, `"revops"`, `"revenue operations"`, `"operações de vendas"`
...
**v5.5 (Claude-2 #2):** a ordem de execução canônica do Passo 12 está consolidada na tabela **§11.3.0** ("Ordem canônica do Passo 12"). Este anexo apontava para uma versão em prosa que fica desatualizada quando a §11.3.0 é modificada. Para evitar duas fontes de verdade, a ordem autoritativa é a tabela em §11.3.0 com 17 sub-passos numerados (12.1 a 12.17 — v5.7 adicionou smoke do Trigger N3 como 12.17).
