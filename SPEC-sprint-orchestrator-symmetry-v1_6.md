# SPEC — Sprint Orchestrator Symmetry v1.6

**Projeto:** CalibraCV
**Versão:** orchestrator-symmetry-v1.6
...
- Migração de evolução de `event_names`: separação `role_*`/`skill_*` por construção via update determinístico baseado em `metadata->>'entity_type'` pré-existente. Aplica a `canonical_promoted_dynamic` → `role_promoted_dynamic`/`skill_promoted_dynamic` e `canonical_creation_blocked_low_confidence` → `role_creation_blocked_low_confidence`/`skill_creation_blocked_low_confidence`. Atualização paritária de `CHECK constraint` em `events.event_name` se houver. Update de call sites TS que emitem os eventos antigos para emitir os nomes novos (§2.y)
- Triggers paritários `trg_jcr_emit_canonical_role_created` em `job_canonical_roles` e `trg_jcs_emit_canonical_skill_created` em `job_canonical_skills`, ambos `AFTER INSERT` emitindo `canonical_role_created`/`canonical_skill_created` em `events` com `metadata = {canonical_id, label, created_via}` (§2.z)
...
- Helper `handleSkillItemError` paritário ao `handleItemError` pré-existente, em `lib/pipeline/batch-processor/error-handlers.ts`, com prefixo de log `[batch]` (convenção por módulo, não por entidade) (§3.2)
...
- Refator do shape JSONB de `dashboard_daily_summary.data`: aggregator atual emite famílias top-level uppercase `data.O7`/`data.O8`/`data.O9` (role-only); v1.6 reorganiza para família aninhada lowercase `data.operational.{o7, o8, o9}` (role pré-existentes) + paralelos novos `data.operational.{o7_skill, o8_skill, o9_skill, items_processed}`. Refator coordenado entre aggregator (§3.6.2), backfill 26 dias e UI dos painéis 2.7/2.8/2.9 (§5.2) no mesmo SUB-PR para evitar janela de inconsistência
...
- **Auditoria de outros painéis admin que ainda não usam `dashboard_daily_summary`** (fora dos 10 Limiares) — frente futura sistêmica. Ver LK-PS-21.
...
### §0.5 Sequência de execução — 11 SUB-PRs

A sprint orchestrator-symmetry é sequencial após a cleanup v3.4 ter sido mergeada (main + dev). 11 SUB-PRs com dependências internas explícitas.

| SUB-PR | Frente | Conteúdo | Depende de |
|---|---|---|---|
| 1 | A | §2.1 (FOSI com CHECK + raw_item_index + UNIQUE + canonical_status) + §2.2 (colunas skill em FOR) + §2.3 (validação simétrica) + auditoria scripts/funções (§6.1, §6.2) | cleanup v3.4 mergeada |
| 2 | A | §2.x corretivos retroativos em FORI (CHECK 0..1, raw_item_index + UNIQUE, ENUMs, CASCADE) + §2.4 RPC `count_skill_items_by_status` + §2.6 rename FORI (5 funções reais) + §2.y migração event_names role_*/skill_* + §2.z triggers canonical_*_created + §3.4 call sites do rename + atualização emissores TS de events | SUB-PR 1 |
| 3 | A | §3.1 refator descoberta retornando `DiscoverResultDetailed` + §3.2 `insertFOSkillItem` individual e bulk (incluindo `canonical_status`) + paridade bug fix retroativo em FORI + `handleSkillItemError` em error-handlers.ts | SUB-PR 2 |
...
| 6 | B | 12 RPCs SQL paritárias (`count_*_items_by_stage_in_window` + 10 `limiares_panel_N_snapshot` com nomes de events role_*/skill_* + schema correto pipeline_config_history) | SUB-PR 4 (FOSI populada e finalizers escrevendo) + SUB-PR 2 (event_names migrados) |
...
5. Events `canonical_promoted_dynamic` e `canonical_creation_blocked_low_confidence` evoluíram para nomes discriminados por entidade: `role_promoted_dynamic` / `skill_promoted_dynamic` e `role_creation_blocked_low_confidence` / `skill_creation_blocked_low_confidence`. Update determinístico das rows existentes baseado em `metadata->>'entity_type'`. Call sites TS que emitem esses eventos atualizados para emitir os nomes novos. `CHECK constraint` em `events.event_name` (se existir) atualizada para incluir os 4 nomes novos e remover os 2 antigos.
6. Triggers paritários `trg_jcr_emit_canonical_role_created` em `job_canonical_roles` e `trg_jcs_emit_canonical_skill_created` em `job_canonical_skills`, ambos `AFTER INSERT` emitindo `canonical_role_created` / `canonical_skill_created` em `events` com `metadata = {canonical_id, label, created_via}`. Aggregator passa a ler séries não-vazias para drift O7 role+skill.
...
8. `handleSkillItemError` paritário ao `handleItemError` pré-existente em `lib/pipeline/batch-processor/error-handlers.ts`. Catch de exceções upstream em `safeDiscoverAndLinkSkills` invoca `handleSkillItemError` para cada skill que falhou, registrando item em FOSI com `status='failed'` + `pipeline_stage='fallback_error'`. `insertEvent('persist_curation.skill_map_failed')` pré-existente mantido (auditoria global em `events`).
...
-- Inclui 'success' para paridade com FORI (que usa 'success' em process-item.ts:200)
...
CREATE TABLE function_orchestrator_skill_items (
  ...
  skill_raw_kind text NOT NULL CHECK (skill_raw_kind IN ('hard', 'soft', 'tool', 'cert')),
...
COMMENT ON COLUMN function_orchestrator_skill_items.needs_review IS
  ...
  'do tipo (hard/soft/tool/cert) tem baixa confiança. Não bloqueia processamento; '
  'serve para curadoria manual posterior.';
```

**Critério de aceite (§7.2):** tabela criada com 5 índices + 2 CHECK constraints + 1 UNIQUE index parcial; `pg_constraint` reporta `chk_foski_skill_confidence_range` ativo; `pg_indexes` reporta `uq_foski_run_posting_index` com filtro `(raw_item_index IS NOT NULL)`.

### §2.2 — `function_orchestrator_runs` ADD colunas de skill

**Arquivo:** `02_function_orchestrator_runs_add_skill_counters.sql`
...
**Arquivo:** `03_function_orchestrator_role_items_retroactive_constraints.sql`
...
**Arquivo:** `04_evolve_event_names_role_skill.sql`

**SUB-PR:** 2 (paralelo a §2.x corretivos retroativos).
...
- Discriminação entidade está em `metadata->>'entity_type'` (`'role'` ou `'skill'`)
- Eventual `CHECK constraint` em `events.event_name` (se houver) precisa ser ampliada para incluir os 4 nomes novos e remover os 2 antigos pós-migração

```sql
BEGIN;

-- 1. Pré-check: todas as rows dos 2 nomes antigos têm metadata.entity_type válido?
DO $$
DECLARE
  v_missing int;
BEGIN
  ...
  WHERE event_name IN ('canonical_promoted_dynamic', 'canonical_creation_blocked_low_confidence')
    AND (metadata IS NULL
         OR metadata->>'entity_type' IS NULL
         OR metadata->>'entity_type' NOT IN ('role', 'skill'));
  IF v_missing > 0 THEN
    RAISE EXCEPTION 'Migration abortada: % rows sem metadata.entity_type válido', v_missing;
  END IF;
END$$;

-- 2. Atualizar CHECK constraint em events.event_name (se existir) — adicionar 4 novos antes de update
-- Lista exata de nomes válidos vem do §6.1 ground truth via pg_constraint
-- Padrão esperado: events_event_name_check
-- Estratégia: DROP CHECK antigo → UPDATE rows → ADD CHECK novo (sem ambos os nomes antigos)

ALTER TABLE events DROP CONSTRAINT IF EXISTS events_event_name_check;

-- 3. Update determinístico baseado em metadata.entity_type
UPDATE events
...
-- 4. Re-criar CHECK constraint com os 4 nomes novos + demais event_names pré-existentes
-- (lista completa vem do §6.1; placeholder mínimo abaixo — substituir pela lista real do ground truth)
ALTER TABLE events ADD CONSTRAINT events_event_name_check
  CHECK (event_name IN (
    -- Nomes novos paritários desta sprint
    ...
    -- + demais event_names pré-existentes do projeto (lista completa via §6.1 ground truth)
  ));
...
BEGIN
  ...
  IF v_old_count > 0 THEN
    RAISE EXCEPTION 'Migration incompleta: % rows ainda têm nomes antigos', v_old_count;
  END IF;
END$$;
...
**Critério de aceite (§7.2):** `SELECT COUNT(*) FROM events WHERE event_name IN ('canonical_promoted_dynamic', 'canonical_creation_blocked_low_confidence')` = 0; rows com nomes novos discriminados por entidade; `pg_constraint` reporta `events_event_name_check` com lista atualizada; smokes S17a/S17b (eventos role/skill emitidos com nomes novos) verdes.

### §2.z — Triggers paritários `canonical_role_created` / `canonical_skill_created`

**Arquivo:** `05_triggers_canonical_role_skill_created.sql`
...
-- Função do trigger lado SKILL (paritária)
...
-- Triggers AFTER INSERT em ambas as tabelas
...
**Coluna `created_via`:** ground truth via §6.1 (E0a expandido) verifica se a coluna existe em `job_canonical_roles` / `job_canonical_skills`. Se não existir, ajustar a função trigger para usar valor estático `'pipeline'` em vez de `COALESCE(NEW.created_via, 'unknown')`.

**Critério de aceite (§7.2):** smoke S19 (inserção sintética de canonical role/skill emite eventos corretos) verde; `pg_trigger` reporta ambos triggers ativos; `pg_get_functiondef` confirma `search_path TO 'public', 'pg_temp'` em ambas funções; após smoke S19, `SELECT COUNT(*) FROM events WHERE event_name='canonical_role_created'` e idem para skill > 0.

### §2.3 — Validação de schema simétrico entre `_role_items` e `_skill_items`

**Arquivo:** `04_validate_symmetric_schema.sql`

**SUB-PR:** 1.

```sql
-- Validação: 18 colunas em function_orchestrator_skill_items
-- (id, run_id, job_posting_id, raw_item_index, skill_raw_name, skill_raw_kind,
...
**Arquivo:** `05_rpc_count_skill_items_by_status.sql`
...
**Pré-requisito:** SUB-PR 1 finalizado — `AUDIT-orchestrator-symmetry-v1.5-<data>.md` lista exata de objetos a renomear/atualizar via §6.2.
...
-- Lista exata validada via §6.1 E0a:
...
ALTER INDEX fo_items_pipeline_stage_idx
  RENAME TO function_orchestrator_role_items_pipeline_stage_idx;

ALTER INDEX fo_items_created_at_idx
  RENAME TO function_orchestrator_role_items_created_at_idx;
...
**Validação obrigatória pós-rewrite (smoke S20 — §7.2):**
...
- `lib/pipeline/types.ts` — tipo `RawSkill` ampliado com `needsReview?: boolean`

#### §3.1.0 — Estado atual: dois cenários possíveis (resolução via E0a) + caller já em `await`

Sessões anteriores de spec referenciavam `discoverAndLinkSkills` retornando `result { skills_reused, skills_pending_created }`. A cleanup v3.4 referencia `safeDiscoverAndLinkSkills` retornando void. Antes de iniciar o refator, evidence E0a da §6.1 confirma qual cenário vige no codebase atual:
...
**Diretriz crítica:** ground truth via E0a revelou que o caller em `lib/pipeline/persist-curation/persist-fn.ts:410` JÁ ESTÁ EM `await safeDiscoverAndLinkSkills(supabase, normalized);` — não é mais fire-and-forget. O comentário inline `M60 — await both; emit events on failure` indica que houve refator anterior (sprint M60) que converteu o caller para `await` mas a documentação não foi atualizada. **Logo, esta sprint NÃO converte caller de fire-and-forget para `await` — caller já está em `await`. Refator é exclusivamente: (a) expandir tipo de retorno do wrapper de `Promise<void>` para `Promise<DiscoverResultDetailed>`; (b) usar o valor retornado no caller para empurrar para `insertFOSkillItemsBatch`.**

Ambos os cenários convergem para o mesmo resultado funcional. A diferença é apenas qual arquivo recebe a expansão de tipo de retorno e em quantas camadas. O ground truth via E0a determina isso antes da implementação começar.
...
export type SkillProcessingDetail = {
  ...
  skill_raw_kind: 'hard' | 'soft' | 'tool' | 'cert';
...
Tipo `RawSkill` em `lib/pipeline/types.ts` ampliado para incluir `needsReview?: boolean` (propagação D-PS-69 da cleanup v3.4 end-to-end):

```typescript
export type RawSkill = {
  label: string;
  kind: 'hard' | 'soft' | 'tool' | 'cert';
  confidence: number;
...
**Validação de `skill_confidence`:** helper `validateSkillConfidence(value)` análogo a `validateCanonicalConfidence` pré-existente para role; clampa para [0,1] retornando `null` se fora da faixa. Defesa em profundidade contra parser bug ou LLM retornando valor fora da faixa — combinado com `CHECK chk_foski_skill_confidence_range` em §2.1.
...
try {
  ...
  // Mas registrar como item failed para visibilidade no orchestrator (paritário a handleItemError).
  ...
  skillDetail = {
    ...
    items: skillsPassedGate.map((s, idx) => ({
      raw_item_index: idx,
      skill_raw_name: s.label,
      skill_raw_kind: s.kind,
...
const gateRejectedItems: SkillProcessingDetail[] = skillsRejectedByGate.map((s, idx) => ({
  raw_item_index: skillsPassedGate.length + idx,
  skill_raw_name: s.label,
  skill_raw_kind: s.kind,
  canonical_skill_id: null,
  canonical_skill_proposed: null,
  canonical_status: null,
  pipeline_stage: 'gate_rejected' as const,
  skill_confidence: s.confidence,
  similarity_score: null,
  reasoning: null,
...
await insertFOSkillItemsBatch(ctx, runId, jobPostingId, allItems);
```

**Cenário B (wrapper `safeDiscoverAndLinkSkills` + função interna `discoverAndLinkSkills`) — confirmado pela cleanup v3.4 como vigente em produção:**
...
export async function safeDiscoverAndLinkSkills(
  ...
  try {
    ...
    // Manter insertEvent pré-existente para auditoria global em `events`
    await insertEvent(supabase, 'persist_curation.skill_map_failed', {
      job_posting_id: normalized.jobPostingId,
      error: err instanceof Error ? err.message : String(err),
    });
    return {
      aggregate: { extracted: 0, reused: 0, pending_created: 0, gate_rejected: 0, failed: normalized.skills.length },
      items: normalized.skills.map((s, idx) => ({
        raw_item_index: idx,
        skill_raw_name: s.label,
        skill_raw_kind: s.kind,
        canonical_skill_id: null,
...
- Como `skillsRejectedByGate` está estruturado em `persist-curation/persist-fn.ts` hoje — precisa carregar `label` + `kind` + `confidence` + `needsReview` para reconstruir o item de orchestrator com `raw_item_index` único.
- Como `RawSkill` é declarado em `lib/pipeline/types.ts` — precisa incluir `needsReview?: boolean` para propagar D-PS-69 da cleanup v3.4. Se ainda não inclui, ampliar declaração.
- `validateSkillConfidence(value)` em `lib/pipeline/persist-curation/validators.ts` (helper novo).

**Critério de aceite (§7.2):** TSC limpo; `discoverAndLinkSkills` retorna array com 1 entry por skill processada em cada path; caller invoca `insertFOSkillItemsBatch` quando `allItems.length > 0`; smoke S15 (propagação `needs_review`) verde.
...
import type { BatchContext, SkillProcessingDetail } from '../types';
...
export async function insertFOSkillItem(
  ...
  const { error } = await ctx.supabase
    ...
    .insert({
      ...
      skill_raw_kind: detail.skill_raw_kind,
...
export async function insertFOSkillItemsBatch(
  ...
  const rows = details.map(d => ({
    ...
    skill_raw_kind: d.skill_raw_kind,
...
Caller de `insertFOItem` em `process-item.ts:200` atualizado para passar `rawItemIndex` (índice do item no batch processado).

**Helper `handleSkillItemError`** em `lib/pipeline/batch-processor/error-handlers.ts` (paritário ao `handleItemError` pré-existente):
...
export async function handleSkillItemError(
  ...
  jobPostingId: string,
  skillRawName: string,
  skillRawKind: 'hard' | 'soft' | 'tool' | 'cert',
  rawItemIndex: number,
  errorType: string,
  errorDetail: string,
): Promise<void> {
...
**Critério de aceite (§7.2):** TSC limpo; smoke `insertFOSkillItemsBatch` com 10 rows válidos popula 10 entries em FOSI com `raw_item_index` distintos; smoke de duplicação (mesma combinação `run_id+job_posting_id+raw_item_index`) retorna erro de UNIQUE constraint sem corromper estado.
...
- `lib/pipeline/batch-processor/process-item.ts:200` — INSERT em `function_orchestrator_items`
...
AS $$
  ...
  SELECT jsonb_build_object(
    'series_role_events', COALESCE(jsonb_agg(jsonb_build_object('day', re.day, 'cnt', re.cnt) ORDER BY re.day) FILTER (WHERE re.day IS NOT NULL), '[]'::jsonb),
    'series_skill_events', COALESCE((SELECT jsonb_agg(jsonb_build_object('day', day, 'cnt', cnt) ORDER BY day) FROM skill_events), '[]'::jsonb),
    'series_skill_fosi', COALESCE((SELECT jsonb_agg(jsonb_build_object('day', day, 'cnt', cnt) ORDER BY day) FROM skill_fosi), '[]'::jsonb)
  )
  FROM role_events re;
$$;
```
...
AS $$
  WITH unified AS (
    SELECT 'role' AS entity, WIDTH_BUCKET(confidence_median, 0.0, 1.0, 10) AS bucket, COUNT(*) AS cnt
    FROM job_canonical_roles
    WHERE confidence_median IS NOT NULL
      AND status IN ('active', 'pending')
    GROUP BY 1, 2
    UNION ALL
    SELECT 'skill' AS entity, WIDTH_BUCKET(confidence_median, 0.0, 1.0, 10) AS bucket, COUNT(*) AS cnt
  ...
  SELECT jsonb_build_object(
    'distribution_role', COALESCE((SELECT jsonb_agg(jsonb_build_object('bucket', LEAST(bucket, 10), 'cnt', cnt) ORDER BY bucket) FROM unified WHERE entity='role'), '[]'::jsonb),
    'distribution_skill', COALESCE((SELECT jsonb_agg(jsonb_build_object('bucket', LEAST(bucket, 10), 'cnt', cnt) ORDER BY bucket) FROM unified WHERE entity='skill'), '[]'::jsonb)
  );
$$;
```
...
AS $$
  WITH role_candidates AS (
    SELECT jsonb_agg(
      jsonb_build_object('id', id, 'similarity', similarity, 'created_at', created_at)
      ORDER BY similarity DESC
    ) AS rows
    FROM (
      SELECT id, similarity, created_at
      FROM canonical_role_merge_candidates
      WHERE created_at >= NOW() - p_interval::INTERVAL
      ORDER BY similarity DESC
      LIMIT 100
    ) sub
  ),
  ...
  SELECT jsonb_build_object(
    'stuck_role', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'days_stuck', EXTRACT(DAY FROM NOW() - created_at)::int
...
AS $$
  WITH role_gap_days AS (
    SELECT COALESCE((SELECT value::INT FROM pipeline_config WHERE key='role.retirement.gap_days'), 90) AS d
  ),
  skill_gap_days AS (
    SELECT COALESCE((SELECT value::INT FROM pipeline_config WHERE key='skill.retirement.gap_days'), 180) AS d
  )
  SELECT jsonb_build_object(
    'retire_role', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
...
AS $$
  SELECT jsonb_build_object(
    'histogram_role', (
      ...
      FROM (
        SELECT LEAST(WIDTH_BUCKET(confidence_median_at_promotion, 0.0, 1.0, 10), 10) AS bucket, COUNT(*) AS cnt
        FROM job_canonical_roles
...
AS $$
  SELECT jsonb_build_object(
    'histogram_role', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object('bucket', bucket, 'cnt', cnt) ORDER BY bucket), '[]'::jsonb)
      FROM (
        SELECT LEAST(WIDTH_BUCKET(creation_confidence, 0.0, 1.0, 10), 10) AS bucket, COUNT(*) AS cnt
        FROM job_canonical_roles
    ...
    'histogram_by_path_skill', (
      ...
      FROM (
        SELECT fosi.pipeline_stage::text AS path,
               LEAST(WIDTH_BUCKET(fosi.skill_confidence, 0.0, 1.0, 10), 10) AS bucket,
               COUNT(*) AS cnt
        FROM function_orchestrator_skill_items fosi
        JOIN function_orchestrator_runs r ON r.id = fosi.run_id
...
SET search_path TO 'public', 'pg_temp'
AS $$
  -- Ground truth via §6.1: tabela pipeline_config_history tem colunas reais
  -- previous_value (não old_value), changed_by (não actor_id), changed_at (não created_at)
  SELECT jsonb_build_object(
    'recent_changes', COALESCE(jsonb_agg(jsonb_build_object(
      'id', id, 'key', key, 'old_value', previous_value, 'new_value', new_value,
      'actor_id', changed_by, 'reason', reason, 'created_at', changed_at
...
**Nota:** os aliases dentro do `jsonb_build_object` (`'old_value', previous_value` etc.) preservam o **shape externo** do JSONB que a UI consome — UI continua lendo `old_value`/`actor_id`/`created_at` no payload. A divergência fica encapsulada na RPC e respeita ground truth do schema real do banco. Tradeoff: assimetria visível entre nomes do banco e nomes do payload, mas evita refator da UI sem ganho funcional.
...
AS $$
  WITH promotions AS (
    SELECT
      ...
      CASE WHEN event_name LIKE 'role_%' THEN 'role' ELSE 'skill' END AS entity,
      COUNT(*) AS cnt
    FROM events
...
async function aggregatePipelineOrchestratorMetrics(
  ...
  return {
    ...
    skill: {
      ...
      o7_canonicals_novos: skillCanonicalsNovos,
      o7_drift_percent: o7SkillDrift,
      o9_fallback_error: fallbackErrorCount,
      o9_sem_canonical: semCanonicalRes.count ?? 0,
    },
...
**Mudança estrutural do JSONB:** o aggregator atual (`aggregator.ts:93-110` em produção) emite famílias top-level **uppercase** `data.O7`, `data.O8`, `data.O9` (role-only) ao lado de `data.ai`, `data.resources`, `data.communications`, `data.auth`, `data.errors`, `data.accumulators` (lowercase, naming pattern padrão das demais famílias). Essa assimetria nominal pré-existe à v1.6 e foi confirmada via ground truth.

v1.6 reorganiza para naming pattern uniforme — todas as famílias em lowercase aninhado:
...
async function aggregateDayData(
  ...
  return {
    // Famílias pré-existentes preservadas (ai, resources, communications, auth, errors, accumulators)
    ...
    operational: {
      ...
      o9_skill: {
        ...
        // status_summary (worst-of-N) é calculado no render layer da UI, NÃO aqui
      },
      items_processed: {
...
- 28 chaves cobertas via estimators (cleanup v3.4 F23 — Caminho B com simulação delta)
...
export type ImpactSource = {
  panel_path: string;      // path no JSONB de dashboard_daily_summary
  histogram_path?: string; // path no JSONB para histograma (subset com sample_size suficiente)
  current_value_filter?: 'gte' | 'lte' | 'eq';
};
...
export const IMPACT_SOURCES: Record<string, ImpactSource> = {
  ...
  'role.promotion.lookback_days': {
    panel_path: 'data.limiares.panel_10.promoted_role',
  },
  'skill.promotion.lookback_days': {
    panel_path: 'data.limiares.panel_10.promoted_skill',
  },
  ...
  // OPUS_REVIEW (2) — derivado de pipeline_config_history + opus_arbitration_outcomes
  'role.opus_review.cooldown_days': { panel_path: 'data.limiares.panel_9.recent_changes' },
  'skill.opus_review.cooldown_days': { panel_path: 'data.limiares.panel_9.recent_changes' },
  // RETIREMENT (2)
  'role.retirement.gap_days': { panel_path: 'data.limiares.panel_6.retire_role' },
  'skill.retirement.gap_days': { panel_path: 'data.limiares.panel_6.retire_skill' },
  // CONFIDENCE (4) — cobertura via estimator base da v3.4 (sem histograma derivado)
  'role.confidence.lookback_days': { panel_path: '' },
  'skill.confidence.lookback_days': { panel_path: '' },
  'role.confidence.min_count': { panel_path: '' },
  'skill.confidence.min_count': { panel_path: '' },
  // AUTO_ASSIGN_FAMILY (2 role-only — assimetria D-PS-41 cleanup v3.4)
  'role.auto_assign_family.min_similarity': { panel_path: '' },
  'role.auto_assign_family.min_score': { panel_path: '' },
};

// Validação de cobertura: 28 chaves cobertas pela cleanup v3.4 F23
...
```json
{
  ...
  "affected_count": 8,
  "affected_label": "canonicals podem entrar em janela de revisão",
  "projected_event_cost_usd": null,
  "cost_window": null,
  "cost_is_fallback": null,
  "panels": ["Aba Limiares — Painel 9 (Histórico pipeline_config)"],
...
Para `role.auto_assign_family.min_similarity` e `role.auto_assign_family.min_score` — cobertas por estimator pré-existente da cleanup v3.4 §3.16 mas **fora de `IMPACT_SOURCES`** por D-PS-41 cleanup v3.4 (assimetria de famílias só role; sem painel Limiares 6 para skill consequentemente sem histograma derivado pelo `dashboard_daily_summary`):
...
```json
{
  ...
  "system_key_reason": "Chave de sistema, não exposta na tela /admin/pipeline-config.",
  "current_impact": null,
  "proposed_impact": null
}
```
...
): 'sufficient' | 'insufficient' | 'estimator_only' | 'system_key' {
...
Critério OR (não AND) — permite reconhecer caso onde N é pequeno mas é 100% do universo (e.g., chave nova com 5 arbitragens totais; 5/5 = 100% e é o melhor sinal possível). Tabela de contagem SEMPRE exibida quando `current_impact` existe. Histograma omitido quando `sample_status === 'insufficient' | 'estimator_only' | 'system_key'`. Edição NÃO bloqueada em nenhum caso — admin tem autonomia.
...
import { estimateImpact } from '@/lib/admin/pipeline-impact-estimators';
...
export async function POST(req: NextRequest, { params }: { params: { key: string } }) {
  ...
  const [estimatorResult, impactResult] = await Promise.allSettled([
    ...
    callImpactSource(supabase, key, current_value, new_value, intervalSql, days),
  ]);
...
async function callEstimator(supabase, key, currentValue, newValue) {
  ...
  // Assinatura real: (admin, oldVal, newVal) => Promise<ImpactEstimate>
...
async function callImpactSource(supabase, key, currentValue, newValue, intervalSql, days) {
...
**Função `aggregateLimiaresHistorical`** em `lib/admin/limiares/aggregate-historical.ts` (arquivo novo) implementa a mesclagem cross-day para cada painel:

- Painéis com histograma (1, 2, 7, 8, 10): soma `cnt` por `bucket`/`day` agrupado
- Painéis com lista (3, 5, 9): concatena listas + dedup por `id`
- Painel 4, 6: snapshot mais recente da janela (estado atual de pending/retirement não muda historicamente)
...
- **Bloco quantitativo (direita):** EXPANDIDO desta sprint. Tabela de contagem (current vs proposto + delta) + histograma + seletor de janela. Para chaves só com estimator (`auto_assign_family.*` role-only — sem `IMPACT_SOURCES`), bloco quantitativo exibe placeholder "Análise quantitativa não disponível para esta chave nesta versão — caixa qualitativa ao lado orienta o impacto."
...
export function ImpactPreview({ configKey, currentValue, newValue }: Props) {
  ...
  // Caso 1: chave coberta apenas por estimator (auto_assign_family.* — role-only)
  ...
  if (data.sample_status === 'system_key') {
    return (
      <div className="text-sm text-muted">
        Chave de sistema — não exposta na interface admin pública.
      </div>
    );
  }
...
- **`ImpactTable.tsx`**: render simples de current/proposed + % e delta absoluto. Quando `current_impact` tem `items_inside_cooldown` em vez de `items_passing` (chaves cooldown), labels adaptam para "Dentro do cooldown" / "Fora do cooldown". Outros adaptadores semânticos por classe de chave: gate → "Aprovados/Rejeitados"; promotion → "Promovidos/Não promovidos"; merge_candidate → "Pares dentro/fora do threshold"; retirement → "Aposentados/Ativos".
- **`ImpactHistogram.tsx`**: SVG inline manual com 10 buckets + 2 linhas verticais (atual e proposto) sobrepondo o histograma. Sem dependência de chart lib (`recharts`/`chart.js`) — escopo limitado não justifica peso extra. Tooltip nos buckets ao hover (ex: "0.70-0.75: 1.245 items, 92% aprovados"). Entrega no MVP.
- **`useDebounce`**: helper pré-existente da cleanup v3.4 (debounce de 500ms no `newValue`), preservado sem mudança.
- **AbortController em request inflight**: preservado da cleanup v3.4 — quando o `newValue` muda durante uma chamada em flight, a anterior é cancelada antes da nova ser disparada.
...
- **Painel 9 (audit trail pipeline_config_history)** — lista cronológica concatenada cross-day, ordenada por `created_at` desc
...
Pré-requisito da Frente A é evidence ground truth (E0) coletado em pre-checks paralelos. Antes do SUB-PR 1 começar, rodar os blocos abaixo e produzir relatório `AUDIT-orchestrator-symmetry-v1.5-<data>.md`:
...
#### E0d — Callers de `discoverSkillsFromFunction` / `function_orchestrator_items`
...
Identificar todos os call sites pré-rename. Auditoria §6.2 cobre o rewrite mecânico.
...
**Entrega:** `AUDIT-orchestrator-symmetry-v1.5-<data>.md` com saída de cada E0a-E0g + decisão de Go/No-Go para cada SUB-PR baseado em estado atual do banco.

#### §6.1.1 — Procedimento geral de auditoria

Além dos blocos E0a-E0g, rodar grep sistemático para mapear todos os pontos de toque desta sprint:
...
grep -rn 'classifyError\|validateCanonicalConfidence\|validateSkillConfidence' lib/ app/ \
  --include='*.ts' >> /tmp/audit-orchestrator.txt
...
**Decisão por categoria (entrega em `AUDIT-orchestrator-symmetry-v1.5-<data>.md`):**
...
**Gate de bloqueio:** SUB-PR 3 só pode iniciar após E0a-E0d ser registrado, com decisão explícita entre Cenário A e Cenário B (ver §3.1.0). SUB-PR 7 só pode iniciar após E0f-E0g + grep 11 ser registrado, com mapa das famílias pré-existentes do `aggregateDayData`. SUB-PRs 8/9 só podem iniciar após grep 10 ser registrado, com localização dos helpers `pg.Pool`/Redis identificada para REMOÇÃO. SUB-PR 10 só pode iniciar após grep 8 + 9 serem registrados, com localização do cache in-process inline e dos componentes `EditModal`/`ImpactPreview` inline confirmadas.
...
**Entrega:** relatório consolidado dentro do `AUDIT-orchestrator-symmetry-v1.6-<data>.md` com cada hit + decisão + snapshot de `proconfig` de cada uma das 5 funções para garantir preservação no rewrite.
...
INSERT INTO function_orchestrator_skill_items
  (run_id, job_posting_id, raw_item_index, skill_raw_name, skill_raw_kind, pipeline_stage, status)
VALUES
...
INSERT INTO function_orchestrator_skill_items
  (run_id, job_posting_id, skill_raw_name, skill_raw_kind, skill_confidence, pipeline_stage, status)
VALUES
...
Cada SUB-PR tem critério de aceite específico (referenciado nas §X.Y respectivas). Os 18 smokes abaixo cobrem ponta-a-ponta o comportamento esperado da sprint.
...
1. Editar `skill.opus_review.cooldown_days` em ambiente com dados suficientes de arbitragem (≥30 outcomes).
...
1. Editar `skill.opus_review.cooldown_days` em ambiente SEM dados suficientes de arbitragem (<30 outcomes na janela).
2. Digitar 60. Aguardar.
3. `ImpactPreview` exibe tabela com `current_impact` e `proposed_impact` (sempre exibida quando current_impact existe) + badge âmbar "Amostra insuficiente para distribuição confiável" mostrando `sample_size` e `sample_threshold`.
4. Histograma omitido.
5. Botão "Salvar" do modal continua habilitado.

#### S10 — Modal lida com chave de auto_assign_family (estimator_only — role-only)

1. Editar `role.auto_assign_family.min_similarity` (estimator pré-existente, fora de `IMPACT_SOURCES` por D-PS-41 cleanup v3.4).
...
1. Editar `CURATE_PIPELINE_ENABLED` via curl direto bypassando UI (`/admin/pipeline-config` filtra essas chaves):
   ...
   ```
2. Payload retorna HTTP 200 com `sample_status='system_key'` + `system_key_reason='Chave de sistema, não exposta na tela /admin/pipeline-config.'`.
...
```sql
BEGIN;
INSERT INTO job_canonical_roles (label, status, created_via)
VALUES ('Smoke Test Role', 'pending', 'manual');
...
```sql
BEGIN;
INSERT INTO job_canonical_skills (label, slug, status, kind, created_via)
VALUES ('Smoke Test Skill', 'smoke-test-skill', 'pending', 'hard', 'manual');
...
4. Rodar smokes E3-E10 da §7.1 e S1-S18 conforme aplicável
5. Rodar `test:sql` 5x consecutivos (gate da §0.2)
6. TSC `--noEmit` limpo (gate da §0.2)
...
|---|---|---|---|---|
| 1 | A | §2.1 FOSI (com canonical_status) + §2.2 colunas skill em FOR + §2.3 validação + §6.1 E0 + §6.2 auditoria (5 funções reais) | 1.5 dia | cleanup v3.4 mergeada |
| 2 | A | §2.x corretivos retroativos FORI (CHECK + raw_item_index + ENUMs + CASCADE) + §2.4 RPC + §2.6 rename + §2.y event_names + §2.z triggers + §3.4 call sites + atualização emissores TS | 2.5 dias | SUB-PR 1 |
| 3 | A | §3.1 refator descoberta + §3.2 insertFOSkillItem (individual+bulk com canonical_status) + paridade bug fix FORI + handleSkillItemError | 1.5 dia | SUB-PR 2 |
...
| 6 | B | 12 RPCs SQL (`count_*_in_window` + 10 `limiares_panel_N_snapshot` com event_names role_*/skill_* + schema correto pipeline_config_history) | 2 dias | SUB-PR 4 + SUB-PR 2 |
...
**Janela total estimada:** 13-15 dias úteis (acréscimo de 1 dia útil vs v1.5 absorve as 3 migrations adicionais — §2.y event_names, §2.z triggers, §2.x conversão ENUMs + CASCADE retroativos).
...
- **D-PS — Migração de evolução de event_names role_*/skill_*:** `canonical_promoted_dynamic` e `canonical_creation_blocked_low_confidence` evoluem para nomes discriminados por entidade. Update determinístico via `metadata->>'entity_type'` pré-existente (D-PS-NEW-3 cleanup v3.4 documentou divergência; v1.6 crava migração). Call sites TS atualizados em paralelo. Discriminação semântica forte; queries de aggregator simplificadas; convenção `<entity>_<verb>_<modifier>` paritária a `canonical_role_created`/`canonical_skill_created`.
- **D-PS — Triggers paritários `canonical_role_created`/`canonical_skill_created`:** cleanup v3.4 cravou os nomes via LK-PS-22 mas trigger nunca foi criado; ground truth via §6.1 confirma 0 rows desses nomes em events. Esta sprint cria 2 triggers paritários (`fn_emit_canonical_role_created` + `fn_emit_canonical_skill_created`) AFTER INSERT em ambas as tabelas, com `metadata={canonical_id, label, created_via, entity_type}`. Sem trigger, drift O7 role+skill seria sempre 0. Paridade por construção.
...
- **D-PS — `handleSkillItemError` extraído em `error-handlers.ts`:** paritário ao `handleItemError` pré-existente. Chamado dentro do catch de `safeDiscoverAndLinkSkills`. NÃO substitui `insertEvent('persist_curation.skill_map_failed')` adicional — soma como dupla camada (event = auditoria global; FOSI = telemetria item-por-item).
- **D-PS — `raw_item_index` corretivo retroativo em FORI:** coluna nova + UNIQUE constraint paritários à criação em FOSI. Update obrigatório em `process-item.ts:200` para passar o índice explicitamente. Idempotência de retry garantida em ambos os lados.
...
- **D-PS — worst-of-N traffic-light no render layer:** classificação visual (verde/âmbar/vermelho) dos painéis O9 e equivalente skill é computada na UI, NÃO no aggregator. Aggregator materializa counters puros (`fallback_error`, `sem_canonical`, etc); UI aplica o algoritmo.
...
- **LK-PS-21** — Auditoria de outros painéis admin que ainda não usam `dashboard_daily_summary` (fora dos 10 Limiares migrados nesta sprint). Frente futura sistêmica.
- **LK-PS-22** — Trigger paritário para emissão de `canonical_skill_created` (paritário ao `canonical_role_created` cravado pela cleanup v3.4). Aggregator desta sprint já usa o nome canônico no filtro de events; emissão pelo lado skill é frente futura caso trigger ainda não exista.
