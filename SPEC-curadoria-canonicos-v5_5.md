...
Esta v5.5 **não** resolve os seguintes problemas, deliberadamente:
...
│ • SEM fallback por embedding nesta v5.5                      │
└─────────────────────────────────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────────────┐
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'CREATE TABLE skill_merge_decisions_backup_v5_5 AS
                 SELECT *, NOW() AS backup_taken_at FROM skill_merge_decisions';
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'CREATE TABLE role_merge_decisions_backup_v5_5 AS
                 SELECT *, NOW() AS backup_taken_at FROM role_merge_decisions';
...
**Problema:** a Camada 0 depende de `original_title` da âncora refletir o título bruto **no momento da ingestão**. Se um admin editar o campo `title` de uma vaga via endpoint admin futuro, `original_title` pode acabar sobrescrito ou dessincronizado — quebrando o propósito do guard de título (que compara `candidateTitle` de vaga nova com `original_title` da âncora).

**Mitigação v5.5:** trigger `BEFORE UPDATE` que rejeita mudanças em `original_title` e `original_description` após o INSERT inicial. A primeira gravação (pelo pipeline de ingestão ou pelo backfill inline §5.1) é livre. Qualquer UPDATE subsequente nessas colunas levanta EXCEPTION.
...
AS $$
BEGIN
    -- Só protege se a coluna JÁ tinha valor (não bloqueia o primeiro INSERT)
    IF OLD.original_title IS NOT NULL
       AND NEW.original_title IS DISTINCT FROM OLD.original_title THEN
        RAISE EXCEPTION 'original_title é imutável após INSERT inicial. '
            'Tentativa de alterar de % para % na vaga %',
            OLD.original_title, NEW.original_title, OLD.id
            USING ERRCODE = 'check_violation';
    END IF;

    IF OLD.original_description IS NOT NULL
       AND NEW.original_description IS DISTINCT FROM OLD.original_description THEN
        RAISE EXCEPTION 'original_description é imutável após INSERT inicial. '
            'Tentativa de alterar na vaga %',
            OLD.id
            USING ERRCODE = 'check_violation';
    END IF;
...
CREATE TRIGGER trigger_enforce_original_immutability
...
COMMENT ON FUNCTION enforce_original_immutability IS
    'v5.5 (DeepSeek #1) — garante imutabilidade de original_title e original_description após a primeira gravação. Previne que endpoints admin de edição de título quebrem o guard de título da Camada 0 (§7.3, N7).';

COMMIT;
```

**Casos que o trigger permite:**
- INSERT inicial com `original_title` e `original_description` populados pelo pipeline de ingestão (§5.1.1)
- UPDATE que altera `title` (mutável) deixando `original_title` intacto
- Backfill inline da §5.1 que popula de NULL para valor inicial

**Casos que o trigger rejeita:**
- UPDATE que tenta alterar `original_title` de `'Analista Sênior'` para `'Analista de Dados'` — levanta EXCEPTION
- UPDATE que tenta setar `original_title = NULL` quando já tinha valor — levanta EXCEPTION
...
// v5.5 — greenfield, não havia endpoint equivalente no codebase
...
export async function POST(
    ...
    // Validar que o canônico destino existe e está ativo/pending (não archived)
    const { data: destCanonical, error: destErr } = await supabase
        .from('job_canonical_roles')
    ...
    if (destCanonical.archived_at !== null) {
        return NextResponse.json(
            { error: 'Canônico destino está archived. Remap rejeitado.' },
            { status: 400 }
        );
    }

    // Ler estado atual da vaga para evento de auditoria
    const { data: currentJob, error: readErr } = await supabase
        .from('job_postings')
    ...
    // UPDATE atômico: canonical + label + curation_source
    // O trigger N4 (§5.6.3) reage ao valor 'manual_remap' setando human_validated=false
    const { error: updateErr } = await supabase
        .from('job_postings')
        .update({
            canonical_role_id: new_canonical_role_id,
            canonical_role_label: destCanonical.label,
            curation_source: 'manual_remap',  // v5.5 — gatilho do trigger N4
...
**Autorização:** reutiliza `requireAdminAuth` existente em `lib/auth/admin-guard` (confirmado por Claude Code como infraestrutura existente). Admin logado passa, qualquer outro 401.
...
BEGIN
    ...
    THEN
        ...
        -- Evento forense dentro da trigger (função separada para não
        -- bloquear a trigger se insert de evento falhar)
        INSERT INTO events (
            event_name,
            resource_type,
            resource_id,
            ...
            jsonb_build_object('human_validated', OLD.human_validated),
            jsonb_build_object('human_validated', false)
        );
    END IF;
...
COMMENT ON FUNCTION revoke_human_validated_on_manual_remap IS
    'v5.5 (Decisão 4 — greenfield): quando admin faz remap manual de uma vaga previamente human_validated, a validação humana é revogada automaticamente. Remap implica que a validação anterior estava errada — manter human_validated=true propagaria o erro pela Camada 0.';
...
**v5.5 — Deploy unificado PR1 + PR2:** ambiente não está em produção assistida (E2E ainda em estabilização), então a lógica original de "PR1 sai, valida 7 dias, depois PR2" da v5.4 não se aplica. **PR1 e PR2 saem em deploy único e contínuo** — sem janela de validação intermediária em produção. A validação acontece em staging/desenvolvimento antes do merge. Isso elimina também o risco de Category B/D em `domain_synonyms.json` v3.0 apontarem para canônicos que seriam renomeados só no PR2 (GenSpark #1 / Grok #1), porque as operações de taxonomia do Passo 12 executam como pré-requisito do deploy unificado — ver ordem obrigatória do Passo 12 no §11.3 v5.5.
...
- [ ] **v5.5 (Claude-2 polimento) — BLOQUEADOR DURO: Plano Vercel atual é Pro (não Hobby).** Conferir via dashboard Vercel que o projeto está em plano Pro. O handler do `curate-job-postings` declara `maxDuration = 300s`, mas o plano Hobby tem cap de 10s — cron real falha. Se estiver em Hobby, fazer upgrade antes do deploy. **Qualquer outro item do checklist é irrelevante se este não estiver satisfeito** — rollout em Hobby resulta em timeouts silenciosos no primeiro batch grande
...
| 12.14 | **Validação triggers** | `SELECT tgname FROM pg_trigger WHERE tgname IN (...)` retorna 4 linhas (N3, N4, imutabilidade, revogação) | §11.3 |
...
- [ ] **12.14** — Validar triggers: `SELECT tgname FROM pg_trigger` retorna os 4 triggers esperados (N3, N4, imutabilidade, revogação)
...
## Anexo B — Decisões §2.4 cravadas (v5.5 final)
