# Sprint v10.0 — Fluxo de Retorno + Governança de Taxonomia + Limpeza Estrutural
...
--
-- direção do comentário corrigida — o rename real desta migration 
-- é original_description → description (não o contrário).
--
-- v8: preserva NULL para vagas sem descrição bruta original. Trigger de imutabilidade já lida com NULL
-- (NULL != qualquer outro valor → INSERT/UPDATE permitido se for de NULL para valor concreto).
-- Validação `nulls_desc = 0` removida — esperamos NULLs legítimos para vagas históricas pré-fix.
-- 
-- Se houver fonte bruta confiável em `requirements->>'description'` (validar em produção primeiro
-- via spot-check), pode usar como fallback:
--   SET original_description = COALESCE(original_description, requirements->>'description')
-- Mas se não houver, deixar NULL é o comportamento correto.
--
-- Decisão cravada: preservar NULL. Spot-check de requirements fica para sprint posterior.
