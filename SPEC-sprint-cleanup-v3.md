...
CREATE OR REPLACE FUNCTION reconcile_canonical_role(
  ...
  p_source text                    -- sem DEFAULT: caller obrigatório (decisão Claude Code para evitar atribuição silenciosa de source errado)
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
